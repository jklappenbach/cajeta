...
- 🟡 **P6** — stdlib rollout: in progress.
  ...
  - ⏭️ **P6.5 — Stream lambda combinators** (forEach, map, filter, flatMap, fold, reduce, anyMatch, allMatch, noneMatch, ...). Function-typed parameter with type-param substitution (`(T) -> void fn`) doesn't parse in stdlib generics today; grammar/visitor gap to resolve before lambda combinators on Stream<T> can land. Same gap blocks Optional.map/filter/etc. (P6.3).
