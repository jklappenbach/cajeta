...
- 🟡 **Phase 7 — CajetaStruct collapse** (was P2c/P2d; expanded into its own phase). Naive experiment crashed 22/32 shards; this phase does the prep + collapse incrementally.
  ...
  - ⏭️ **P7.3+** — owned-heap-class-ref field in a stack-class crashes (free of invalid pointer) — probably a transient drop entry from the `heap Hello()` expression inside the ctor body that double-fires. Debug needed.
  ...
  - ⏭️ **P7.5** — change `visitStructDeclaration` to create `CajetaClass` instead of `CajetaStruct`; drop `isAggregate` skip in dispatch; full regression.
...
- 🟡 **P6** — stdlib rollout: in progress.
  ...
  - ⏭️ **P6.6** — `T[].stream()` compiler intrinsic. Sugar for `heap ArrayStream<T>(arr, arr.length())`.
