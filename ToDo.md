...
1. **Generic-class-typed field codegen — ~1 session.** `[mode-agnostic]` — When a class field's declared type is a generic-class instantiation (`Stream<T> source;`, `Optional<Hello> value;`), the field is laid out as a by-value struct (`{ ptr }` i.e. `%union.anon` in IR) instead of `ptr`. Breaks ctor signatures + field access. Workaround today: avoid class type parameters as field types. Fix path: in the field-LLVM-type lookup, treat any class-typed field (generic or not) as `ptr`. **Blocks**: P2.2 intermediate combinators (TakeStream/SkipStream/FilterStream/PeekStream all need `Stream<T> source;`), P2.4 `@Encoding<JsonEncoder<X>>`, idiomatic stdlib `Optional<Class>` / `Pair<Class, Class>` usage. Surfaced when P2.2's TakeStream<T> hit "Call parameter type does not match function signature" with `%union.anon` vs `ptr`.
