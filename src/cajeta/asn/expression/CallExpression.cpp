...
namespace cajeta {
    ...
    llvm::Value* CallExpression::generateCode(CajetaModulePtr module) {
        ...
        if (streamExpr) {
            ...
            if (sklass && sklass->toCanonical() == "cajeta.gpu.KernelStream") {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            bool isBuffer = klass &&
                klass->toCanonical().rfind("cajeta.gpu.KernelBuffer", 0) == 0;
            ...
            bool isBufferArray = bufElemKlass &&
                bufElemKlass->toCanonical().rfind("cajeta.gpu.KernelBuffer", 0) == 0;
