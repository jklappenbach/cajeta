...
namespace cajeta {
    ...
    std::vector<std::pair<std::string, std::string>> cacheFlagPairs(
        ...
        // The level, not a bool: `off` and `line` differ in emitted IR, so they must not share a cache key.
        pairs.emplace_back("debug-info", debugInfoName(f.debugInfoLevel));
        // An instrumented build and a plain one must never alias: the difference is a probe pair in every selected method.
        pairs.emplace_back("profiler", profilerName(f.profiler));
        // The selection's CONTENTS, not the path it was read from: keying on the path
        // returns objects probed to a selection the user edited in place.
