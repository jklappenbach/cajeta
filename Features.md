...
| L-07 | `heap` / `stack` allocation prefix | Unified-class storage selection | shipped | `docs/stdlib/UnifiedClasses.md`; `test/parser/UnifiedClassSyntaxTests.cpp` |
| L-08 | Memory model (`#` transfer, drop chain) | Single-owner heap; static + runtime checks | shipped | `docs/stdlib/MemoryModel.md`, `docs/stdlib/FieldOwnership.md`; many parser tests |
| L-09 | Wire-format views (`struct`) | Zero-copy typed overlays; `@BigEndian`/`@Align` | shipped | `docs/stdlib/Views.md`; `test/parser/StructView*.cpp` |
...
| L-12 | `@AutoHash` synthesis | Structural `hash()` walk over fields | shipped | `docs/stdlib/Hashing.md`; `test/parser/AutoHashTests.cpp` |
| L-13 | Annotation argument capture | `Annotatable.getString/Int/Bool/ClassRef/StringList/IntList` | shipped | `docs/stdlib/Annotations.md`; tests across `ComponentRegistrationTests`, `NamedInjectTests`, `OrderChainingTests`, `AnnotationParsingTests` |
...
| L-19 | `super.method()` calls | Direct upcall to parent's implementation | shipped | `test/parser/MultipleInheritanceGapTests.cpp` (superMethodCallReachesParent + explicitSuperCtorWithArgs). `docs/stdlib/UnifiedClasses.md` § super |
| L-20 | Lambda capture by reference | `let captures = ...` semantics | shipped (partial) | `docs/stdlib/Lambdas.md`; full closure semantics in Lambda tests |
...
| L-22 | Method-level type parameters (`final <T> ...`) | Per-method `<T>` declaration | shipped | `docs/stdlib/MethodLevelTemplate.md`; static + instance method templates, inference + explicit-type-arg call syntax (`xs.map<int64>(...)` — Form C, type args after the identifier) all working |
...
| ID | Name | Description | Status | Doc / tests |
|----|------|-------------|--------|-------------|
| S-101 | `Object` base | Universal root; identity defaults | shipped | `docs/stdlib/Lang.md` |
...
| S-103 | `String` owned vs view modes | `viewOf`, `toOwned`, lifetime tying | designed | `docs/stdlib/Lang.md` |
| S-104 | `String.fromCodePoints` / `.repeat` / `.lines` / `getBytes` / `codePointAt` / `compare` / `lastIndexOf` | Less common String surface | designed | `docs/stdlib/Lang.md` |
...
| ID | Name | Description | Status | Doc / tests |
|----|------|-------------|--------|-------------|
| S-201 | `Stream<T>` base + `next()` protocol | Pull-protocol root + default empty | shipped | `docs/stdlib/Streams.md` |
...
| S-204 | Stream terminal: `collect(Collector<T, R>)` | Needs S-501 Collector | designed | `docs/stdlib/Streams.md` |
...
| S-213 | `TakeWhileStream` / `DropWhileStream` | Predicate-based take/skip | designed | `docs/stdlib/Streams.md` |
| S-214 | `DistinctStream` / `EnumerateStream` / `ZipStream` / `ChainStream` / `SortedStream` / `WindowedStream` | Remaining intermediate combinators | designed | `docs/stdlib/Streams.md` |
...
| S-403 | `Hasher` interface | Streaming `writeT(...)` + `finish()` contract | designed | `docs/stdlib/Hashing.md` |
...
`docs/stdlib/Time.md`. Tests: `test/time/` (JIT). All value types are
...
All `cajeta.io.*` is **designed, unimplemented**. See `docs/stdlib/Io.md`,
`IoFile.md`, `IoNet.md`.
...
| ID | Name | Description | Status | Doc / tests |
|----|------|-------------|--------|-------------|
| S-801 | Fiber runtime (R1-R5-A') | Stackful fibers + single-carrier cooperative scheduler. NOT YET: work-stealing pool, timer wheel, I/O reactor | shipped | `docs/stdlib/AsyncStatus.md` |
...
| S-805 | R5-C / R5-D runtime items | R5-C cooperative cancellation + R5-D scope exception-escalation, in the runtime (`__cajeta_fiber_cancel`, cancel re-raise in `__cajeta_task_wait`) | shipped | `docs/stdlib/AsyncStatus.md` |
...
| ID | Name | Description | Status | Doc / tests |
|----|------|-------------|--------|-------------|
| S-1001 | `@Component` / `@Inject` DI | Dependency injection annotations | shipped | `docs/stdlib/AspectModel.md`; `test/parser/ComponentRegistrationTests.cpp`, `InjectCodegenTests.cpp`, `NamedInjectTests.cpp` |
| S-1002 | `@Before` / `@After` / `@Around` advice | Aspect-oriented method wrapping | shipped | `docs/stdlib/AspectModel.md`; `test/parser/AdviceCodegenTests.cpp`, `AroundAdviceTests.cpp`, `AfterReturningThrowingTests.cpp` |
| S-1003 | Pointcut matching | Method-name + annotation matchers | shipped | `test/parser/PointcutMatchingTests.cpp` |
| S-1004 | `@Inject` vs implicit injection | Decision: keep `@Inject` or drop in favor of auto | pending | `docs/stdlib/AspectModel.md` |

## Annotations

See `docs/stdlib/Annotations.md` for the full catalog. The annotation-
