...
| L-03 | Multiple inheritance | Class extends multiple classes/interfaces; required for "container IS a stream" pattern | partial | grammar/vtable/dispatch/super/Gap 8 layout/poly-MI dispatch shipped: per-parent sub-object layout, this-offset adjustment at parent ctor/method call sites, upcast pointer adjustment at LocalVariableDeclaration sites, secondary vtables for non-first-parent sub-objects with synthesized offset thunks for cross-class overrides. Remaining gap: true diamond / virtual-base machinery (E extends B,C where both extend A duplicates A's sub-object when A isn't on both first-parent chains). Other upcast sites (assignment expression, return statement, parameter passing) are not yet threaded through `adjustForUpcast` — only the local-variable declaration path is wired. Pinned by `test/parser/MultipleInheritanceGapTests.cpp` and `test/parser/DynamicDispatchTests.cpp`. `cajeta-docs/stdlib/Streams.md`, Collections.md |
