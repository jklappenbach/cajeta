...
The public documentation website for Cajeta. Built on Astro + Tailwind
(AstroWind template), but **all documentation content is sourced live from the
canonical `docs/` tree at the repository root** — there are no copied markdown
files in this project. This is the single source of truth: edit the specs under
`../docs/`, rebuild, and the site reflects them. The published docs cannot drift
from the in-repo specs.
...
- `src/utils/docs.ts` derives each page's title (first `# ` heading), description
  (first prose paragraph), and category from the file body + path — the docs/
  markdown has no frontmatter.

To add or change a doc, edit the file under `../docs/`. To add a top-level nav
link, edit `src/navigation.ts` (links use the lowercase slugs above).
...
- The AstroWind **blog** is disabled (`src/config.yaml` `apps.blog.isEnabled:
  false`); its utilities remain because `src/utils/permalinks.ts` references
...
- `public/decapcms/` is the template's CMS admin (geared at the now-disabled
  blog); it isn't linked from the site.

The AstroWind template reference follows.
