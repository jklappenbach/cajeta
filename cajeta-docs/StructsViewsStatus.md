...
**Current line item:** Session 6 complete (S6.1–S6.6). Structs lay out, init via brace syntax, hold class refs, drop correctly with ownership-transfer + alias-as-move semantics, integrate with the borrow / path-borrow tracker, and reject variable-tail fields with clear messages. Next: Session 7 (inline composition — struct as a class field).
