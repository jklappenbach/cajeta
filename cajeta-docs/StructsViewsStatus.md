...
**Current line item:** Session 6 complete. Session 7 complete. Session 8 in progress (S8.1, S8.2, S8.3 done). Next: S8.4 (struct method may return a value of the enclosing struct's own concrete type) — `return this;` already works; constructing a fresh same-type instance (`return Point { ... };`) fails because Method::generatePrototype's return-type detection sees a placeholder rather than the real CajetaStruct for the enclosing struct.
...
- [ ] **8.4** Struct method may return a value of the enclosing struct's own concrete type for direct-call use; restriction only applies later under interface dispatch (S11.2). Two shapes: `return this;` (return the receiver itself — already works via S6.7's ReturnStatement double-load branch for struct-typed named locals, where `this` is one such field) and `return MyStruct { ... };` (construct a fresh same-type instance — currently fails LLVM verify because `Method::generatePrototype`'s same-struct return-type lookup hits a placeholder rather than the real CajetaStruct, leaving the function signature declared as `ptr` while ReturnStatement emits a struct value). The terminology "Self" in earlier doc revisions was informal shorthand for "the enclosing struct's own concrete type" — cajeta has no `Self` keyword and doesn't need one (`this` covers the receiver-return case; the concrete type name covers fresh-instance returns).
