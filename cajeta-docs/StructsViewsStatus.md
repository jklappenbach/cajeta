...
**Current line item:** S6.1 + S6.2 + S6.3 + S6.4 complete — structs lay out, init via brace syntax, hold class refs, and participate in the drop chain. Next: S6.5 (borrow checker for paths into struct fields).
...
- Every class-ref binding in an aggregate initializer is treated as a move regardless of whether the source carried `#`. The borrow form (struct field holding a borrowed class ref whose drop the struct must skip) needs per-instance ownership tracking that the current chain shape doesn't carry — implementing it cleanly means either a per-construction custom drop fn or a side-channel bitmask. Practical impact today: `Holder h = Holder { t: tag };` makes the struct treat tag as owned; using `tag` after the binding is a soundness gap (struct's drop will free what's still aliased through `tag`). S6.5's borrow checker should mark such locals as moved-from at the binding site so the gap can't be reached.
