...
- [ ] **11.2** Reject method calls through interface value when the method's return type is the implementing struct's own concrete type (`CAJETA_ERROR_DYN_DISPATCH_OWN_TYPE_RETURN`). The interface value doesn't carry the concrete size, so a same-concrete-type return value has nowhere to land. Direct calls on the concrete struct keep working — only the through-interface path is rejected.
...
- [ ] **11.5** 8 new tests: dispatch to struct impl, dispatch to class impl, dispatch in a polymorphic array iteration, same-concrete-type return through interface rejected, direct same-concrete-type return on concrete struct still works, method called through interface value matches direct-call result, vtable hit count verified, mixed class/struct in same Vec dispatches correctly.
