...
- **Executor** — currently a single carrier OS thread. R8.2 replaced the global mutex-protected linked-list ready queue with a Chase-Lev work-stealing deque (`cajeta_carrier_deque`): atomic `top` / `bottom` sequence numbers over a fixed-size circular slot array, single-producer / multi-consumer protocol. The owner-side (`push_bottom`, `pop_bottom`) drains the deque LIFO — the cache-warm end. The `steal` operation is implemented (FIFO from the deque's perspective) but unused under one carrier; R8.3 adds the multi-carrier pool that calls it. Today's pushes still happen under `__cajeta_task_mutex` for the empty-queue condvar handshake; R8.3 lifts the owner-side mutex.
