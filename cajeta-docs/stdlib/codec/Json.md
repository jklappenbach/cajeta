...
- `cajeta-docs/stdlib/Annotations.md` § @ToString — consumes
  `Json.toBytes(this)` when `format=TO_STRING_JSON`.
- `cajeta-docs/stdlib/Annotations.md` § @Encoding — binary-format
  codegen (MessagePack / Protobuf / Avro). JSON does NOT use
  `@Encoding`; see § "Why not `@Encoding(JsonEncoder<T>)`" above.
- `cajeta-docs/stdlib/ErrorModel.md` — `JsonException`'s place in
  the Recoverable hierarchy.
- `cajeta-docs/stdlib/Streams.md` — `JsonArray` / `JsonObject`
  multiple-inherit `Stream<...>`.
- `cajeta-docs/stdlib/MethodLevelTemplate.md` — `Json.parse<T>` /
