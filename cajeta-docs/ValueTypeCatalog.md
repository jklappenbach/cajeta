...
| Operator | Form | Signature | Notes |
|---|---|---|---|
| `load` | instance-mutating-unary | `public void load(Buffer<T> src, uint32 offset, uint32 layout, uint32 stride)` | Instance method (mutates this in-place with the loaded data). Device lowering emits OpCooperativeMatrixLoadKHR via llvm.spv.cooperative.matrix.load. The `offset`/`stride` select a sub-tile of a wider matrix, so a K-loop of `load`+`mma` is a tiled GEMM (device-verified on AMD WMMA). Storage class is always Global (Buffer<T>) for now; a Workgroup (LDS) staged load source is deferred pending a design discussion (surface + staging strategy). |
