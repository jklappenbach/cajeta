...
> **Cross-references:** [`CompilerModes.md`](CompilerModes.md) for the existing mode presets, [`stdlib/MemoryModel.md`](stdlib/MemoryModel.md) for the drop-chain design, [`stdlib/FieldOwnership.md`](stdlib/FieldOwnership.md) for the live-set rationale, [`stdlib/Thread.md`](stdlib/Thread.md) for the concurrency primitives that need a threading abstraction layer.
...
4. **Direct `malloc` / `free`.** `__cajeta_alloc` and `__cajeta_free_array` call libc `malloc` / `free`. Many MCU heaps are non-libc (FreeRTOS heap, TLSF, region allocators) or absent entirely.
5. **Direct `fprintf` / `printf` to stderr.** `__cajeta_log` uses `fprintf(stderr, ...)`. No stdio on bare-metal; the user wants UART, semihosting, RTT, ITM, or nothing.
...
2. **Custom panic / abort.** Today `__cajeta_panic` calls libc `abort` (which triggers SIGABRT, the dump handler, and exit). Bare-metal wants something different — typically a debug-trap (`BKPT`), a soft reset, or a watchdog-controlled hang. Need pluggable.
