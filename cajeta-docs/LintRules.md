...
- **`wildcard-materialize-in-loop`** — A wildcard-typed expression (`Stream<?>`, `Optional<?>`, etc.) has `next()` / `get()` / similar element-producing method called on it inside a loop body, and the result is bound to a local or passed onward. Wildcard dispatch goes through the template-relative vtable hash; the receive-side dispatch is one indirect call per iteration, the inliner can't see through it, and LLVM loses the unroll/vectorize opportunity. The "10–20× larger loop body" framing in `TemplateWildcard.md`. **Suppress when:** the wildcard cursor is chain-walker code that only calls type-erased `unwrap`/`splittableSize`/`trySplitRoot` (those don't materialize T). **Don't suppress when:** the loop is the actual element-processing hot path. Downcast at the worker boundary first (per Step 5b chain-walker pattern) so the hot loop stays specialized. See `cajeta-docs/TemplateWildcard.md` § Performance.
- **`wildcard-crosses-hot-boundary`** — A method whose return type is wildcard (`Stream<?>`, `Optional<?>`, `T<?>`) is invoked from a body the compiler can identify as hot (inside a loop, inside a per-element callback, inside a parallel-stream worker fn). Wildcard returns can't be specialized at the call site — every receiver use that wants the concrete type pays a downcast and loses the inline opportunity. **Suppress when:** the boundary is a one-shot per stream (chain-walker setup, splittable-root pick) rather than per-element. **Don't suppress when:** the wildcard return is the per-element value itself; restructure to either return concrete or keep the wildcard inside the abstraction.
- **`wildcard-field-in-small-class`** — A class declares a wildcard-typed field (`Stream<?> source`, `Optional<?> cached`) AND the class itself is allocated in a hot path (per-element wrappers, per-iteration result holders, frequently-constructed entries). Per the Step 2 drop ABI, wildcard fields drop through `__cajeta_class_virtual_drop` — every drop is an indirect call plus a vtable load, where a concrete-field equivalent would emit a direct call (inlinable). For a class allocated millions of times, the difference is measurable. **Suppress when:** the class is allocated infrequently (cached, pooled, long-lived). **Don't suppress when:** the class is in the construction hot path; pick a concrete element type or push the wildcard outward.
- **`discarded-wildcard-next`** — A `.next()` (or equivalent element-producing) call on a wildcard-typed receiver is invoked in statement position, the returned `#Optional<?>` is not assigned, and the call is not a side-effect-only chain step. The boxed Optional allocates on the heap, the wildcard dispatch costs an indirect call, and nothing observes the result — both costs are wasted. **Suppress when:** the call is intentionally used for its side effect (uncommon — `peek` exists for that). **Don't suppress when:** the result is genuinely discarded; remove the call or restructure.
