...
- ~~**`Box<int32>.get()` returns garbage in multi-module builds**~~ — **fixed.** When a use-site referenced a templated class whose declaration hadn't parsed yet, fromContext's placeholder-synthesis path created a placeholder without `typeParameters`, so `templateClass->isTemplate()` returned false and the instantiation branch silently skipped — dropping the type arguments. Fix extends the prescan to capture per-class `typeParameters` + raw source text, seeds those onto the placeholder so `isTemplate()` returns true and `instantiate(args)` has what it needs, and makes the instantiation cache process-wide via `structureToModule` so the same `T<args>` instance is reused regardless of which module first triggered the build.
- ~~**`System.out` (Java idiom) silently compiles to a no-op**~~ — **fixed.** The compiler now throws `CAJETA_ERROR_UNKNOWN_SYSTEM_STREAM` with a "did you mean `stdout`?" suggestion (Levenshtein over the known stream names, gated on `--diag-hints`).
- ~~**`printf("template {}", arg)` produces no output in binary emit**~~ — **fixed.** The printf intrinsic expects `(String fmt, String[] args)`; passing a scalar second arg silently dropped the call. The compiler now throws `CAJETA_ERROR_PRINTF_BAD_ARGS` naming the bad type and pointing at the right shape. String concatenation (`"prefix " + value`) remains the easier formatter for ad-hoc printing.
