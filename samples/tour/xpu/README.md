...
- `reluReg` / `pruneReg` — **branchless per-lane conditionals (mask → select)**.
  ...
  Full walkthrough + the alternatives it replaces: `docs/MaskSelect.md`.
- `quatk` / `linalgk` — **3-D transform algebra on the device**. `quatk` runs a
  ...
  AMD. Walkthroughs: `docs/Quaternions.md`,
  `docs/MatrixDeterminantInverse.md`.
- `coopGemm` — **the matrix cores + the tiering fallback**. One 16×16×16 tile
  ...
  `cajeta.xpu.core.CooperativeMatrix` + `docs/LintRules.md` § Notes.
...
See `docs/gpu/xpu/CajetaCPU.md` and `docs/gpu/xpu/CajetaXPU.md`
