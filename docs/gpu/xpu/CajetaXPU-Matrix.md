...
> 9 trailing `i32` coordinate params; `GpuThread`/`Workgroup` reads pull from those
...
> (`CUDA → HIP → Vulkan → CPU`) and routes launch + `GpuBuffer<T>` device memory to
...
> projected the Vulkan column on the assumption that **GpuBuffer Device Address**
...
|---------|------|--------|-----|--------|
| GpuThread (local) id `x/y/z` | one `threadId(dim)` seam method | `native` · `nvvm.read.ptx.sreg.tid.*` | `native` · `amdgcn.workitem.id.*` | `native` · `llvm.spv.thread.id.in.group` (LocalInvocationId) |
...
| Feature | Core | NVIDIA | AMD | Vulkan |
|---------|------|--------|-----|--------|
| `GpuBuffer<T>` arg as raw device pointer | `addrspace(1) T*` function param | `native` · pointer in `kernelParams[i]` | `native` · pointer in `kernelParams[i]` | `abstraction: ` **descriptor-set SSBO** — each `GpuBuffer<T>` is an `OpVariable StorageBuffer` (set 0, binding = arg index) accessed via `llvm.spv.resource.handlefrombinding` → `getpointer`. (BDA was the intended path but is **not possible** — see the row below.) |
...
| Why not GpuBuffer Device Address | — | — | — | `not possible: ` LLVM 23's SPIR-V backend exposes **no PhysicalStorageBuffer/BDA intrinsic** — the `PhysicalStorageBuffer64EXT` strings are capability enum names only. So raw-pointer kernel args can't be reconstituted from IR; descriptor sets are the only model. |
...
the build proved it's *not* bridgeable by GpuBuffer Device Address (no IR path),
...
| Dispatch + sync | `GpuStream.sync()` | `native` · `cuLaunchKernel` + sync | `native` · launch + `hipDeviceSynchronize` | `forks: ` `vkQueueSubmit` + `vkQueueWaitIdle` |
...
| `@Device` user-defined helper calls | ✅ scalar + GpuBuffer<T> params, same-class, helper-chains; alwaysinline-folded per backend (Vulkan: `bufferParamType` = the storage-buffer handle + AlwaysInliner before SPIR-V isel); verified on AMD & Vulkan |
...
- **Vulkan · GpuBuffer Device Address (raw pointers)** → LLVM 23's SPIR-V backend has **no PhysicalStorageBuffer/BDA intrinsic** from IR. This forced the descriptor-set model above — the central build-discovered finding. *(§3)*
...
**portable software BVH** (the `SoftwareRayQuery` walk over a `GpuBuffer<float32>`,
