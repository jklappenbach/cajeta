...
- **Texture dimensions** — `Texture1D/2D/3D/2DArray` + fetch/sample work on CPU + Vulkan/RADV + AMD/gfx1151 on-device (NVIDIA emit-only). **`TextureCube` sample and mipmapped `Texture2D` + explicit-LOD (`sampleLod`) work on CPU + Vulkan/RADV**, but on **AMD they are blocked by the HIP runtime, not the hardware**: `hipMalloc3DArray[hipArrayCubemap]` → invalid-arg and `hipMallocMipmappedArray` → `hipErrorNotSupported(801)` on gfx1151, **confirmed unchanged across ROCm 7.2.2 and 7.11.0** by a standalone HIP probe (layered/2D/3D arrays alloc fine). The same GPU does both via the Vulkan/RADV path, so **use the Vulkan backend for cubemaps / mip-LOD on AMD**. cajeta's lowering + ockl `_lod`/`_CM` seams are correct and light up if ROCm implements the array types. *(see project memory `reference_amd_device_jit_comgr_collision` neighbourhood / `XpuHipDispatchDeviceTests` skips)*
