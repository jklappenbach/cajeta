...
```
cajeta-gpu  (foundation: value types, Buffer, Image, AccelerationStructure, atomics,
...
|------|----------------|-------------|---------------|
| **Noun** | `SplatCloud` — build-description (SoA attribute buffers + metadata); built acceleration (depth order, tile bins, AABB BVH, octree LOD) follows the build, never converted | compressed/octree GPU residency; hardware-BVH for the ray path | uncompressed SoA in `Buffer<T>`; software LBVH; CPU reference |
...
Stored **Structure-of-Arrays** (one `Buffer<T>` per attribute) — required for the sort
