...
A user annotation is declared with the `annotation` keyword. Its methods are its elements; an element may declare a default.
...
- **Synthesis** (`cajeta.synth`) — `@Getter`, `@Setter`, `@ToString`, `@EqualsAndHashCode`, `@Data`, `@Value`, `@Builder`, `@NoArgsConstructor`, `@AllArgsConstructor`, `@RequiredArgsConstructor`, `@With`, `@AutoHash`: the compiler synthesizes the corresponding members; a user-written member of the same signature wins over the synthesized one.
...
`@Order(n)` sequences multiple aspects matching the same method; `@NoAdvice` opts a method out of weaving. An `@Aspect` class is itself a `@Component`, so advice can `@Inject` collaborators.

**Weaving guarantees.** Weaving happens at compile time — no runtime proxies and no load-time instrumentation. For `@Around`, the original body is extracted into a private helper and the public method *becomes* the woven wrapper; consequently there is no self-invocation bypass — `this.method()` inside the class reaches the advice like any external call. Overrides inherit advice through virtual dispatch.
...
The DI *substrate* is core language; only policy above it is framework. The compiler owns a compile-time graph:
