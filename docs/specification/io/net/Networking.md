# Networking — `cajeta.io.net.http` / `cajeta.io.net.ws`

> **Status — forward design.** What ships **today** is a leaner HTTP/1.1
> client + server and an RFC 6455 WebSocket, **in stdlib** under
> `cajeta.io.net.http` and `cajeta.io.net.ws` — documented in
> [`docs/Net.md`](../../../Net.md), verified against
> [`runtime/src/cajeta/io/net/http/`](../../../../runtime/src/cajeta/io/net/http/)
> and [`.../ws/`](../../../../runtime/src/cajeta/io/net/ws/). This document
...
A design for `cajeta.io.net.http` covering HTTP/1.1 + HTTP/2 + WebSockets,
both client and server, with TLS for HTTPS / WSS. Built on `cajeta.io.net`
...
`cajeta.io.net` (sockets) is what most networked code needs at the
...
- Programs that don't touch HTTP get no HTTP bytes — LLVM DCE drops the
  unused `cajeta.io.net.http` / `cajeta.io.net.ws` code, so the import is
...
- WebSockets sit next to HTTP (`cajeta.io.net.ws`) since WS rides the HTTP
...
cajeta.io.net.http.X` / `import cajeta.io.net.ws.X`.
...
cajeta.io.net.ws     — Frame protocol, message types, control
                            frame handling, close codes
cajeta.io.net.ws.client — WebSocket client (handshake + frame
                            loop)
cajeta.io.net.ws.server — WebSocket server-side upgrade
...
10. **cajeta.io.net.ws.** Frame protocol (RFC 6455). Just
...
11. **cajeta.io.net.ws.server.** Upgrade integration with
    cajeta.io.net.http.server. WebSocket endpoints reachable.
12. **cajeta.io.net.ws.client.** WebSocketClient. Standalone,
    independent of server work.
13. **cajeta.io.net.ws: permessage-deflate (RFC 7692).**
