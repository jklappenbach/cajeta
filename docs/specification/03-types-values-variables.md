...
A class is one type regardless of where its instances are placed: `stack MyClass()` and `heap MyClass()` produce values of the same type `MyClass`, and placement is chosen at the allocation site, not in the declaration (Allocation §4). The borrow checker tracks lifetime; the type system does not.
