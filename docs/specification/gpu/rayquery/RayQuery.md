...
This is the design for making *inline ray query* a genuine `cajeta.xpu` feature —
...
*pipeline* — but as an IMPLEMENTATION of the same `cajeta.xpu` inline `RayQuery`
...
`cajeta.xpu` inline ray query runs on **every** backend: native `OpRayQuery*KHR`
