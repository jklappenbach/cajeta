...
- **Tensor<T,Dims...>** — _now_ — N-dimensional array value type for ML workloads (the workhorse of SPELA/Toffee).
...
- **Purpose:** N-dimensional array value type for ML workloads (the workhorse of SPELA/Toffee). Parameterized by element type T and shape dimensions (Dims = compile-time shape constants, e.g., Tensor<float32, 64, 32, 16> for a 3-D 64×32×16 tensor). Represents dense numeric data: matrices, batched matrices, feature maps, activations. On host: stack/heap allocation, element/slice access, elementwise + reductions. On device: GPU register/shared/global residency, cooperative-matrix binding, matmul/conv kernels.
