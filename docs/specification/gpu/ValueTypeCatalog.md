...
> companion to [`OperatorOverloading.md`](../lang/OperatorOverloading.md).
>
...
- **Tensor<T,Dims...>** — _now_ — N-dimensional array value type for ML workloads (the workhorse of SPELA/Caramelo).
...
- **Depends on:** KernelBuffer<T> (global device-memory binding), Non-type template parameter substrate (uint32 Rows/Cols/Use constants), float16 element type (IEEE binary16 — remapped from prior bfloat misnomer in CM5a)

| Operator | Form | Signature | Notes |
|---|---|---|---|
| `load` | instance-mutating-unary | `public void load(KernelBuffer<T> src, uint32 offset, uint32 layout, uint32 stride)` | Instance method (mutates this in-place with the loaded data). Device lowering emits OpCooperativeMatrixLoadKHR via llvm.spv.cooperative.matrix.load. The `offset`/`stride` select a sub-tile of a wider matrix, so a K-loop of `load`+`mma` is a tiled GEMM (device-verified on AMD WMMA). `load`/`store` come in a global `KernelBuffer<T>` and a workgroup-shared `Shared<T>` (LDS) overload; the LDS overload + `CoopStage.panel` cooperative staging give the full LDS-staged GEMM (device-verified on AMD WMMA, CPU, and Vulkan/RADV — the Vulkan path needed two cajeta-llvm SPIR-V backend fixes: array-global typing + access-chaining an aggregate cooperative-matrix pointer to element 0). |
...
| `store` | instance-mutating-unary | `public void store(KernelBuffer<T> dst, uint32 offset, uint32 layout, uint32 stride)` | Instance method. Device lowering emits OpCooperativeMatrixStoreKHR via llvm.spv.cooperative.matrix.store. |

**Methods:** `public void load(KernelBuffer<T> src, uint32 offset, uint32 layout, uint32 stride)`, `public void splat(T value)`, `mma(…)`, `public void store(KernelBuffer<T> dst, uint32 offset, uint32 layout, uint32 stride)`
...
- **mask** (on a `Vector<boolean,N>` from a comparison): `all()`/`any()` → `boolean`, `select(whenTrue, whenFalse)` → per-lane blend. **Masks are register-only** — `KernelBuffer<Vector<boolean,N>>` / bool-vector kernel args stay ABI-rejected.
...
> **Status (2026-06-05) — LIVE (host + CPU/Vulkan), a compiler intrinsic like Vector/Matrix.** `CajetaQuaternion` carries `QUATERNION_FLAG | PRIMITIVE_FLAG`, lowering to a flat `<4 x T>` = (w, x, y, z); **float element only** (`CAJETA_ERROR_QUATERNION_ELEMENT_TYPE`). Construction `new/stack Quaternion<T>(w,x,y,z)`. Operators (intrinsic, the table below is documentary): `*` = Hamilton product `Quaternion*Quaternion` and vector rotation `Quaternion*Vector<T,3>` (→ rotated `Vector<T,3>`); `+ -` element-wise. Methods: `normalize()`, `conjugate()`, `length()`, `dot(other)`, and the interpolators **`slerp(other, t)`** (constant angular velocity; `acos`/`sin` via the device transcendental seam, branchless with an automatic near-parallel `nlerp` fallback) and **`nlerp(other, t)`** (cheaper normalized lerp). Both run on CPU/Vulkan/AMD. The `==`→boolean row below is superseded by the value-type comparison-mask rule (`MaskSelect.md`) and is **not implemented for Quaternion in v1**.
