# 01 — Installation

Two ways in: `cvm` (the toolchain manager) or building from source.

## With cvm

`cvm` installs and switches Cajeta toolchains the way `rustup` does for Rust.
It lives in [`tools/cvm`](../../tools/cvm/), and ships as a prebuilt binary
with every release.

### Get cvm

<!-- BEGIN:CVM_DOWNLOAD — generated by scripts/update-release-docs.sh, run from the release workflow. Anything between these markers is replaced on the next release. -->
Download the build for your platform, mark it executable, and put it on your
`PATH`. It is a single self-contained binary with nothing to install
alongside it.

| Platform | Triple | Download |
|---|---|---|
| Linux x86_64 | `x86_64-linux-gnu` | [`cvm-v0.30.0-x86_64-linux-gnu`](https://github.com/jklappenbach/cajeta/releases/download/v0.30.0/cvm-v0.30.0-x86_64-linux-gnu) |
| Linux ARM64 | `aarch64-linux-gnu` | [`cvm-v0.30.0-aarch64-linux-gnu`](https://github.com/jklappenbach/cajeta/releases/download/v0.30.0/cvm-v0.30.0-aarch64-linux-gnu) |
| macOS Apple Silicon | `aarch64-apple-darwin` | [`cvm-v0.30.0-aarch64-apple-darwin`](https://github.com/jklappenbach/cajeta/releases/download/v0.30.0/cvm-v0.30.0-aarch64-apple-darwin) |
| Windows x86_64 | `x86_64-w64-mingw32` | [`cvm-v0.30.0-x86_64-w64-mingw32.exe`](https://github.com/jklappenbach/cajeta/releases/download/v0.30.0/cvm-v0.30.0-x86_64-w64-mingw32.exe) |

```bash
curl -fsSL -o cvm https://github.com/jklappenbach/cajeta/releases/download/v0.30.0/cvm-v0.30.0-x86_64-linux-gnu
chmod +x cvm && sudo mv cvm /usr/local/bin/
```

Verify the download against the matching `.sha256` published beside it.
These links point at `v0.30.0`; the
[latest release](https://github.com/jklappenbach/cajeta/releases/latest) always carries
the current build.
<!-- END:CVM_DOWNLOAD -->

### Use cvm

```bash
cvm install latest      # download, verify checksum, install, activate
cvm default <version>   # switch the active toolchain
cvm which               # show the cvm home + active shim
cvm doctor              # diagnose the environment cvm sees
```

Toolchains install under `~/.cajeta/versions/<version>/`; the active `cajeta`
shim goes on your `PATH`. `cvm self update` updates cvm itself.

## From source

```bash
git clone https://github.com/jklappenbach/cajeta.git
cd cajeta
./setup.sh     # toolchain prerequisites (LLVM, ANTLR, CMake, Ninja)
./build.sh     # builds the compiler
```

The binary lands at `build/src/cajeta`. On Windows use `setup.cmd` and
`build.cmd`.

## Verify

```bash
$ cajeta --version
cajeta 0.8.0 (8190ee59)
```

Any version string means you're installed. Platform packages (`.deb`, `.rpm`,
`.msi`, macOS bundles) are on the roadmap; today it's cvm or source.

Next: [Kick the tires](02-kick-the-tires.md).
