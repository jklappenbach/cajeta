...
| Cross-process, bidirectional / structured IPC | a Unix-domain socket (preferred; reuses the `cajeta.net` socket + reactor machinery) | a FIFO |
...
public final class Pipe {
    ...
    // are non-blocking and registered with the cajeta.net reactor, so reads and
...
- **Anonymous pipes are portable.** Linux uses `pipe2(O_NONBLOCK | O_CLOEXEC)`;
  ...
  readiness/completion rides the existing `cajeta.net` reactor (epoll / kqueue /
...
- **FIFOs are POSIX-only.** Windows "named pipes" (`\\.\pipe\…`,
  ...
  (`cajeta.net`, including Windows AF_UNIX) is the preferred path for portable
