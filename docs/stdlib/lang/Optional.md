...
`orElse`). `get()` on an empty Optional returns the zero-initialized unset
value rather than throwing — guard with `isPresent()`.
...
| `T get()` | Extract the value; the zero-initialized unset value when empty |
