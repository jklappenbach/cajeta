...
Cajeta. Strings are read-only: every transformation (`substring`, `trim`,
`toUpperCase`, `replace`, ...) returns a fresh heap `String` rather than
mutating in place (hence the `#String` returns), while query methods (`count`,
`size`, `contains`, `indexOf`, `startsWith`, `codepointAt`, ...) read without
allocating. Equality is content-based: `==` dispatches through the `hash()`
override, and `equals(String)` is the explicit byte-for-byte check — so
strings key cleanly into `HashMap` and `HashSet`.
...
| `String(#int8[] bytes, int32 byteLength)` ⚑ | View-mode constructor — borrows the caller's byte buffer without claiming ownership |
| `int64 hash()` | Content-based hash overriding Object's identity hash |
| `int64 count()` | Number of Unicode characters (codepoints) |
...
| `static boolean operator== (String a, String b)` | Content equality operator |
...
| `#String substring(int32 begin, int32 end)` | Half-open byte-indexed slice `[begin, end)` |
...
| `#String trim()` | Strips leading and trailing ASCII whitespace |
