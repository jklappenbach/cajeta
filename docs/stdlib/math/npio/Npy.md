...
element bytes in C-order. Four dtypes are supported, each a typed save/load
pair (cajeta is statically typed, so the caller picks the loader matching the
file's dtype): `float32` `'<f4'`, `float64` `'<f8'`, `int32` `'<i4'`,
`int64` `'<i8'`.
