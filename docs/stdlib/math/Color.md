...
sRGB-encoded texture/UI space and linear light space. A value type over four
`float32` channels (r, g, b, a), each in `[0,1]`. Shading must happen in
linear light, but textures and UI colors are usually sRGB-encoded:
`toLinear()` decodes sRGB → linear, `toSrgb()` encodes linear → sRGB, both
using the standard IEC 61966-2-1 piecewise transfer function. Alpha is linear
