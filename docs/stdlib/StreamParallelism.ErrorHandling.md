...
task semantics are `Concurrency.md` and `ErrorModel.md § Async / fiber
...
the worker explicitly polls — see `Concurrency.md § Cooperative
...
carrier lands, the counter wants `AtomicInt32` (Concurrency.md §
...
## §6 Cross-references
  ...
  - `Concurrency.md § Cooperative cancellation`, `Concurrency.md § Atomics` —
