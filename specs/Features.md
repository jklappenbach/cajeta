...
| L-22 | Method-level type parameters (`final <T> ...`) | Per-method `<T>` declaration | shipped | `docs/specification/lang/templates/MethodLevelTemplate.md`; static + instance method templates, inference + explicit-type-arg call syntax (`xs.map<int64>(...)` — Form C, type args after the identifier) all working |
