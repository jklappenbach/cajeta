...
| [compiler-jsonl](compiler-jsonl-spec.md) | [plan](../agents/compiler-jsonl-plan.md) | active — approved 2026-07-31; Units 1-6 ALL delivered (envelope + `kind` everywhere; version negotiation; levelled `log` + terminal `result`; `--diag-format` uniform across verbs; console dispatches on `kind`; schema + anti-rot guard). Unit 7 (live-pass fixes: plugin passes the flag; buildJit sets diagFormat so syntax errors are located diagnostics; DAP error carries a message; DAP narration + plugin notices are records) delivered b035ab0d, 125 tests green. Remaining: live passes 5.3.1/5.3.2/7.3.2 (Julian) — plugin installed + content-verified 18:05 |
