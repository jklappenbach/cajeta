...
| [uniform-transfer-semantics](uniform-transfer-semantics-spec.md) | — (needs a plan) | **draft — APPROVED IN PRINCIPLE by Julian 2026-08-02, not started.** A field should behave like a variable: `v #= obj.field.v` takes ownership, so `#` on the right of `#=` is redundant everywhere and `x #= #y` becomes a blanket error. Requires containers to OWN their elements (20 fused claims, 10 element formals -> `#T`, 42 stdlib lending call sites, plus cajeta-ml + cajeta-xgboost). API-BREAKING, cannot be staged — the diagnostic and the container change must land together. The bare-identifier half already shipped (409d8ffe). |
