...
| [moved-local-borrows](moved-local-borrows-spec.md) | — (plan pending spec approval) | draft — reconciles `MemoryModel.md` rows 536 and 543, which state opposite outcomes for the same hazard: a use-after-free reached through the moved NAME is rejected, the same one reached through an ALIAS is programmer responsibility. Makes borrow-reads of a moved-from local legal; re-transfer stays `USE_AFTER_MOVE`. Explicit non-goal: move-while-borrowed (Rust E0505) — Julian's ruling 2026-08-02 is not to box the language in for a hazard row 543 already assigns to the developer. Unblocks uniform-transfer 4.2.1 (reverts to the mechanical migration) and 4.2.4 (resolves toward owning keys). |
