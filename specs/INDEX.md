...
| [compiler-jsonl](compiler-jsonl-spec.md) | [plan](../agents/compiler-jsonl-plan.md) | active — approved 2026-07-31; Units 1-4 delivered (envelope + `kind` everywhere; reader-side version negotiation; narration as levelled `log` records + terminal `result`; `--diag-format` uniform across verbs); next Unit 5 (console dispatches on `kind`) |
