...
| **CP7-6** ✅ | Drop / destructor breakpoints — declared `~Object()` as the root virtual destructor so every class has an inheritable/overridable `~T()`; a breakpoint on a `~T()` body parks at drop via existing safepoints (no new stop path / breakpoint type). Synthesized-wrapper synthetic safepoint + bespoke breakpoint type **dropped by decision**. 2 JIT session tests; risk-suite + 114 debug green | FR-9.1, FR-9.2, FR-9.4, FR-9.5 | stdlib + (existing DAP/plugin) |
