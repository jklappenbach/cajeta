...
- **`olla`** — the registry **service** (server, `/v2` API, web app), in its own
  repo. Details + rationale in **§15.1**.
...
## 15. Registry "Olla" — name, domain, infrastructure

### 15.1 Decision: name & repo boundaries
...
- **Own repository.** Olla (registry *server* + its web/API) lives in its **own
  git repo** (`olla`), separate from this `cajeta` compiler repo. (See §1.1 for
  the repo-boundary decision — why the site stays in-repo while Olla splits out.)
  Rationale:
  different stack (web/TS service vs C++/LLVM), a live service with its own
  security surface and deploy cadence, independent protocol versioning, and the
  "host your own registry" goal. (Published *artifacts* live in object storage,
  not git — §15.4.)
- **Spec stays canonical in `cajeta`.** The wire contract + schemas remain in
  ...
  `lockfile-v1.json`); the build-tool **client** stays in
  `src/cajeta/buildtool/repo/`. The Olla repo *consumes* the spec (git submodule
  or a published schema/test-vector package) and runs **conformance tests**
  against it, so client and server can't drift on the wire format.

### 15.2 Domain scheme (root already owned: `cajeta.dev` @ Namecheap)
- **Canonical root:** **`cajeta.dev`** (already registered at Namecheap).
- **Subdomains** (free under the one root):

  | Host | Serves |
  |------|--------|
  | `cajeta.dev` (apex) | marketing + docs site (`./site`) |
...
**E. The Olla app (Workers + R2 + D1)**
13. Create the **`olla`** repo; scaffold a Worker (or Pages Functions) for the
    `/v2` API + web UI.
...
### 15.4 File storage (artifacts) → Cloudflare R2
...
### 15.5 Catalog DB + search hosting → start serverless
- **System of record: Cloudflare D1** (serverless SQLite, native to Workers).
  ...
  server to run. Powers `resolve`, the version index, and feeds search (§8.2).
...
### 15.5.1 Typo tolerance / close-match (critical requirement)
...
index updated on publish** (index-on-publish hook, §8.2) and **rebuildable from
D1**. Accessed behind the `lib/registry` provider interface so it stays
swappable.
