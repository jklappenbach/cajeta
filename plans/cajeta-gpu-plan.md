...
- [~] **Ray query + acceleration structures** (`SPV_KHR_ray_query`) — **in progress in the `cajeta-spirv` fork branch, TDD per increment.** Was only NV enum stubs. Findings: the zero-param opaque types use the backend's **default `getNonParameterizedType` lowering** (no custom helper needed, simpler than coop-matrix); ray query is **`[EnvVulkan]`-only** (rejected under the OpenCL `spirv64` triple) — so it's inherently the Vulkan/Shader flavor Cajeta emits. Tests live in `llvm/test/CodeGen/SPIRV/extensions/SPV_KHR_ray_query/`.
  ...
  - [ ] **Increment 2b — the operations** (path corrected after a dead end — *important finding*): the `__spirv_*` **builtin-call path does NOT work for ray query.** Verified twice: the builtin interception is gated `if (!STI.isShader())` (`SPIRVPrepareFunctions.cpp:473`) so it runs **only under the OpenCL/Kernel flavor**, while `SPV_KHR_ray_query` is **`[EnvVulkan]`-only** — mutually exclusive. (Attempted it — `DemangledNativeBuiltin` + `generateRayQueryInst` via `buildOpFromWrapper`; built green but the calls stayed `LinkageAttributes Import` under the Vulkan triple. Reverted.) **The ops must use the `llvm.spv.*` intrinsic path** (selected in `SPIRVInstructionSelector`, which runs for every flavor — the same mechanism textures' `llvm.spv.resource.*` use). Corrected touch list:
    - `IntrinsicsSPIRV.td`: `int_spv_ray_query_initialize` (void; rq-ptr, accel, flags, mask, origin, tmin, dir, tmax), `int_spv_ray_query_proceed` (i1; rq-ptr), `int_spv_ray_query_get_intersection_type` (i32; rq-ptr, i32).
    - `SPIRVInstrInfo.td`: the 3 opcodes (Initialize Op<4473> void/8-operand · Proceed Op<4477> bool · GetIntersectionType Op<4479> uint) — *trivial, written-and-validated already in the reverted attempt, just re-add*.
    - `SPIRVInstructionSelector.cpp`: `selectIntrinsic` cases for the three `int_spv_ray_query_*` → `BuildMI(OpRayQuery…)` (rq operand is the `OpVariable` ptr from the alloca; mirror how a resource intrinsic builds its MI).
    - `SPIRVModuleAnalysis.cpp`: op requirement cases (cap+ext) if the type's requirement alone doesn't cover them.
    - Test: `llvm.spv.ray_query.*` intrinsic calls (no mangling needed) → emit the 3 ops; then a spirv-val-clean `GLCompute` kernel with a descriptor-bound AS (the probe's spatial-index pattern).
