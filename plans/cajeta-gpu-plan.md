...
- [x] `Quaternion<T>` type + normalize/rotate (2026-06-05, host + CPU/VK). New `CajetaQuaternion` (`<4 x T>` = w,x,y,z; `QUATERNION_FLAG`), `quatops` helpers. `*` = Hamilton product (Q*Q) / vector rotation (Q*Vector<T,3>); `+ -` element-wise; methods normalize/conjugate/length/dot; interpolation via **nlerp** (branchless, device-capable). **`slerp` deferred** — needs device transcendentals (sin/acos, B2 increment 2); host could ship it but kept symmetric. Float element only (`CAJETA_ERROR_QUATERNION_ELEMENT_TYPE`).
