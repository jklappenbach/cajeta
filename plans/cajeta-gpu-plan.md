...
- [ ] **Ray query + acceleration structures** (`SPV_KHR_ray_query`: `OpTypeRayQueryKHR`, `OpRayQuery*`, `OpTypeAccelerationStructureKHR`) — only NV enum stubs today. **Headline gap; foundation-level (see placement).** Add opaque ext-types (mirror the `spirv.Image`/`spirv.Sampler` `TargetExtType` path), `int_spv_ray_query_*` intrinsics, instruction selection, capability/extension emission (`SPV_KHR_ray_query` already in the symbolic table). Host-side AS build is Vulkan API, not codegen.
