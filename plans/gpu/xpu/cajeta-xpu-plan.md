...
- [x] Function pointers / device-side dispatch (bounded) — a function-typed value is an `i32` TAG over a closed, statically-known set of `@Device`-static candidates; a call lowers to an **if/else-if chain of DIRECT calls** (SPIR-V has no function pointers — this rides identically on all four backends, no backend-specific code). Surfaces on the new host **array-of-function type** `((T)->R)[]` (grouping parens make array-OF-function expressible; `(T)->R[]` binds `[]` to the return): a literal table `((int32)->int32)[] ops = { A::f, B::g }` indexed by a runtime value `ops[i](x)`, plus the single-candidate variable form `(int32)->int32 op = A::f`. Out-of-range index is a **defined no-op** (zero-init result slot, no trap/UB). Device-verified **CPU + Vulkan/RADV + AMD/gfx1151** (`deviceDispatchTableOnDevice` Vulkan + AMD, `deviceDispatchTableOnCpu`, `deviceFnPtrVariableOnCpu`, `deviceDispatchOutOfRangeIsDefinedNoOpOnCpu`); **NVPTX emit-only** (`lowersDeviceDispatchToBranchChain` asserts ZERO indirect calls in the device module + valid PTX). The host type is a full first-class array-of-fn (a real closure-ptr dispatch table) — see `FunctionArrayTypeTests`, `docs/stdlib/Lambdas.md`. **v1-bounded scope:** device-internal dispatch only — candidates are non-capturing `@Device` statics resolved at compile time; host→kernel function-pointer *kernel args* (Stage 12 FFI) and capturing/bound refs on device are deferred. *(Was deferred `XPU-N01`.)*
...
- [~] Kernel specialization constants surfaced to the language — `Spec.geti(slot, default)` reads an i32 specialization constant (compile-time `slot`/`default`), via a `specConstantI32` seam. **Vulkan: a genuine `OpSpecConstant`** (SpecId `kFirstUserSpecId(4)`+slot, override-ready) — LLVM 23 has no spec-constant IR intrinsic, so the lowering emits a Private (addrspace 10) witness global the post-emit pass (`injectUserSpecConstants`) rewrites into the spec constant, mirroring the dynamic-shared-length patch. **CPU/AMD/NVPTX bake the default** (they recompile per launch — no pipeline-time binding). Device-verified CPU + Vulkan/RADV (`specConstantBakesDefaultOnCpu`, `specConstantDefaultOnDevice`, `lowersSpecConstantToOpSpecConstant` — SpecId-4 decoration + spirv-val). **Host-side override deferred to Stage 12** (the launch/FFI contract): the runtime supplies no value for SpecId ≥ 4 yet, so it reads the default. f32 (`getf`) is a follow-up.
