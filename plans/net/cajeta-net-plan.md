...
Companion to [`docs/Net.md`](../docs/Net.md).
...
- [x] **NET-3.5** `AsyncReader`/`AsyncWriter` byte-stream
      ...
      `cajeta.concurrent.AsyncIterator` for chunked consumption.
      `depends-on:` NET-3.3
...
- **NET-1.8** — done. `cajeta.net.NetException` hierarchy (extends `RecoverableException`) + all 10 plan subtypes + `NetErrors.fromErrno` ordinal→subtype mapping (mirrors native `enum cajeta_net_err`) + `docs/stdlib/net/Errors.md` taxonomy; 18 JIT tests.
...
- [~] **(parallel) Compiler-gap fixes** surfaced by the slices, independent of
      ...
      dimension** — `heap T[Klass.STATIC]` / `heap T[this.field]` sext'd a pointer
...
- [ ] **L-16. Full `cajeta_tests` on Linux with the serial-retry pass.** The non-determ-
