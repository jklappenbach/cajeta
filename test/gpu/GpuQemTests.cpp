//
// GpuQemTests — cajeta-gfx plan §2.a (geometry processing foundation), the
// Garland–Heckbert quadric error metric (QEM) math core in cajeta.gpu.Qem. A
...
//
// cajeta.gpu is lazily parsed; importing cajeta.gpu.Qem triggers it.
//
...
const char* IMP = "import cajeta.gpu.Qem;\n";
...
TEST(GpuQemTests, planeErrorIsSquaredDistance) {
...
TEST(GpuQemTests, quadricsAccumulateByAddition) {
    EXPECT_EQ(runI32(IMP,
...
TEST(GpuQemTests, optimalPositionSolvesCorner) {
    EXPECT_EQ(runI32(IMP,
        "        Vector<float32,3> nx = stack Vector<float32,3>(1.0f, 0.0f, 0.0f);\n"
        "        Vector<float32,3> ny = stack Vector<float32,3>(0.0f, 1.0f, 0.0f);\n"
        "        Vector<float32,3> nz = stack Vector<float32,3>(0.0f, 0.0f, 1.0f);\n"
...
TEST(GpuQemTests, optimalPositionFallsBackWhenSingular) {
    EXPECT_EQ(runI32(IMP,
        "        Vector<float32,3> ny = stack Vector<float32,3>(0.0f, 1.0f, 0.0f);\n"
...
TEST(GpuQemTests, edgeCollapseCostMatchesGarlandHeckbert) {
    EXPECT_EQ(runI32(IMP,
        "        Vector<float32,3> ny = stack Vector<float32,3>(0.0f, 1.0f, 0.0f);\n"
        "        Matrix<float32,4,4> q0 = Qem.planeQuadric(ny, 0.0f);\n"
        "        Matrix<float32,4,4> q2 = Qem.planeQuadric(ny, -2.0f);\n"
