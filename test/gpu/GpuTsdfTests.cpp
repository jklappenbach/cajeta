//
// GpuTsdfTests — cajeta-gfx plan §2.c, the truncated signed-distance-function
// voxel grid (cajeta.gpu.Tsdf) with Curless-Levoy fusion. Each voxel holds a
...
const char* IMP = "import cajeta.gpu.Tsdf;\n";
...
TEST(GpuTsdfTests, fuseTwoPlanesToMidpoint) {
...
TEST(GpuTsdfTests, grazingHitIsDownWeighted) {
    EXPECT_EQ(runI32(IMP,
        "        Tsdf g = heap Tsdf(1, 1, 11, 0.1f, 0.5f);\n"
        "        g.fusePlaneZ(0.4f, 1.0f, 1.0f);\n"
...
// d=-0.5) clamps to -0.5.
TEST(GpuTsdfTests, firstFuseStoresMeasurementAndTruncates) {
    EXPECT_EQ(runI32(IMP,
