//
// GpuMeshSimplifierTests — cajeta-gfx plan §2.a, the mesh-level half of the
// Garland–Heckbert edge-collapse simplifier: per-vertex quadric ACCUMULATION
// from an indexed triangle mesh (cajeta.gpu.MeshSimplifier.accumulateQuadrics),
// built on the cajeta.gpu.Qem quadric core. A vertex's accumulated quadric is
...
const char* IMP =
    ...
    "import cajeta.gpu.MeshSimplifier;\n";
...
TEST(GpuMeshSimplifierTests, accumulateSumsIncidentFacePlanes) {
...
TEST(GpuMeshSimplifierTests, accumulationCountsEachIncidentFace) {
    EXPECT_EQ(runI32(IMP,
...
TEST(GpuMeshSimplifierTests, flatRegionCollapsesAtZeroCost) {
    EXPECT_EQ(runI32(IMP,
        "        float32[] pos = heap float32[15];\n"
        "        pos[0] = 0.0f;  pos[1] = 0.0f;  pos[2] = 0.0f;\n"
        "        pos[3] = 1.0f;  pos[4] = 0.0f;  pos[5] = 0.0f;\n"
...
TEST(GpuMeshSimplifierTests, edgeCollapseCombinesEndpointQuadrics) {
    EXPECT_EQ(runI32(IMP,
        "        float32[] pos = heap float32[12];\n"
        "        pos[0] = 0.0f; pos[1] = 0.0f; pos[2] = 0.0f;\n"
        "        pos[3] = 1.0f; pos[4] = 0.0f; pos[5] = 0.0f;\n"
