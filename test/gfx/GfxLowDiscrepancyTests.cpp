//
// GfxLowDiscrepancyTests — cajeta-gfx plan §3 (slice 1): the low-discrepancy
// sampling sequences (cajeta.gpu.LowDiscrepancy).
//
...
const char* IMP = "import cajeta.gpu.LowDiscrepancy;\n";
...
// and 0 -> 0.
TEST(GfxLowDiscrepancyTests, vanDerCorputBase2KnownValues) {
...
TEST(GfxLowDiscrepancyTests, radicalInverseOtherBases) {
    EXPECT_EQ(runI32(IMP,
...
TEST(GfxLowDiscrepancyTests, samplesStayInUnitRange) {
    EXPECT_EQ(runI32(IMP,
        "        int32 i = 1;\n"
        "        while (i < 64) {\n"
...
TEST(GfxLowDiscrepancyTests, haltonAndHammersley) {
    EXPECT_EQ(runI32(IMP,
