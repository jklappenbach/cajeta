//
// GfxPageCacheTests — cajeta-gfx plan §3 (3.d): the generic residency cache
// (cajeta.gpu.PageCache<K,V>).
//
...
// Compile a run() body with cajeta.gpu.PageCache imported and an identity-keyed
// `Tag` class available (a bare class hashes/equals by object identity).
int32_t runPC(const std::string& body) {
    std::string src =
        "package test;\n"
        "import cajeta.gpu.PageCache;\n"
...
TEST(GfxPageCacheTests, lruEvictOrderWithTouch) {
    EXPECT_EQ(runPC(
...
TEST(GfxPageCacheTests, lruEvictsOldest) {
    EXPECT_EQ(runPC(
...
// fallback on a miss.
TEST(GfxPageCacheTests, touchFeedbackAndGet) {
    EXPECT_EQ(runPC(
...
TEST(GfxPageCacheTests, readmitUpdatesNoEvict) {
    EXPECT_EQ(runPC(
        "        PageCache<Tag, int32> pc = heap PageCache<Tag, int32>(2);\n"
        "        Tag a = heap Tag();\n"
        "        Tag b = heap Tag();\n"
