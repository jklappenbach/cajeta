...
// http/chunked.vectors
struct ChunkedVector {
    std::string name;
    std::vector<uint8_t> chunked;   // raw chunked-encoded body
    std::vector<uint8_t> decoded;   // expected decode output
};

inline std::vector<ChunkedVector> chunkedVectors() {
    std::vector<ChunkedVector> v;
    for (auto& r : readRows("http/chunked.vectors")) {
        if (r.size() != 3)
            throw std::runtime_error("golden: chunked.vectors expects 3 fields");
        std::vector<uint8_t> dec = (r[2] == "<EMPTY>")
            ? std::vector<uint8_t>{} : hexDecode(r[2]);
        v.push_back({r[0], hexDecode(r[1]), dec});
    }
    return v;
}

// http/manifest.vectors (+ the sibling .http byte files)
struct HttpMessageVector {
    std::string file;
    std::string kind;          // request | response
    std::string startLine;     // "<METHOD> <target>" or "<status> <reason>"
    std::string framing;       // content-length | chunked | close | none
    long bodyLen;              // decoded length, -1 if streamed/unknown
    bool keepAlive;
    std::vector<uint8_t> bytes; // the byte-exact wire message (CRLF preserved)
};

inline std::vector<HttpMessageVector> httpMessageVectors() {
    std::vector<HttpMessageVector> v;
    for (auto& r : readRows("http/manifest.vectors")) {
        if (r.size() != 6)
            throw std::runtime_error("golden: http manifest expects 6 fields");
        HttpMessageVector m;
        m.file = r[0]; m.kind = r[1]; m.startLine = r[2];
        m.framing = r[3];
        m.bodyLen = std::stol(r[4]);
        m.keepAlive = (r[5] == "1");
        m.bytes = readBytes("http/" + r[0]);
...
// http/abuse.vectors
struct HttpAbuseVector {
    std::string name, generator, expectedFailure;
};

inline std::vector<HttpAbuseVector> httpAbuseVectors() {
    std::vector<HttpAbuseVector> v;
    for (auto& r : readRows("http/abuse.vectors")) {
        if (r.size() != 3)
            throw std::runtime_error("golden: abuse.vectors expects 3 fields");
        v.push_back({r[0], r[1], r[2]});
    }
    return v;
}

// Materialize an abuse-case message from its generator spec. Lets the
// corpus stay tiny while the parser is fed a megabyte-scale flood.
inline std::vector<uint8_t> generateAbuseCase(const std::string& generator) {
    auto bytesOf = [](const std::string& s) {
        return std::vector<uint8_t>(s.begin(), s.end());
    };
    if (generator.rfind("header-flood:", 0) == 0) {
        long n = std::stol(generator.substr(13));
        std::string s = "GET / HTTP/1.1\r\nHost: h.test\r\n";
        for (long i = 0; i < n; ++i)
            s += "X-" + std::to_string(i) + ": v\r\n";
        s += "\r\n";
        return bytesOf(s);
    }
    if (generator.rfind("long-header-line:", 0) == 0) {
        long n = std::stol(generator.substr(17));
        std::string s = "GET / HTTP/1.1\r\nHost: h.test\r\nX-Long: ";
        s.append((size_t)n, 'a');
        s += "\r\n\r\n";
        return bytesOf(s);
    }
    if (generator.rfind("long-request-line:", 0) == 0) {
        long n = std::stol(generator.substr(18));
        std::string s = "GET /";
        s.append((size_t)n, 'a');
        s += " HTTP/1.1\r\nHost: h.test\r\n\r\n";
        return bytesOf(s);
    }
    if (generator == "missing-crlf-terminator") {
        // Headers never terminated by the blank line.
        return bytesOf("GET / HTTP/1.1\r\nHost: h.test\r\nX-A: b\r\n");
    }
    if (generator.rfind("bad-chunk-size:", 0) == 0) {
        std::string sz = generator.substr(15);
        std::string s = "HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n";
        s += sz + "\r\nhello\r\n0\r\n\r\n";
        return bytesOf(s);
    }
    if (generator == "truncated-body") {
        // Content-Length: 100 but only 5 body bytes sent.
        return bytesOf("HTTP/1.1 200 OK\r\nContent-Length: 100\r\n\r\nhello");
    }
    throw std::runtime_error("golden: unknown abuse generator: " + generator);
}

// ws/accept-key.vectors
struct WsAcceptVector {
    std::string name, key, expectedAccept;
};

inline std::vector<WsAcceptVector> wsAcceptVectors() {
    std::vector<WsAcceptVector> v;
    for (auto& r : readRows("ws/accept-key.vectors")) {
        if (r.size() != 3)
            throw std::runtime_error("golden: ws accept expects 3 fields");
        v.push_back({r[0], r[1], r[2]});
    }
    return v;
}

// The RFC 6455 magic GUID the accept-key hash appends.
inline const char* wsAcceptGuid() {
    return "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
}

// ws/frames.vectors
struct WsFrameVector {
    std::string name;
    std::vector<uint8_t> wire;     // full frame bytes
    bool fin;
    uint8_t opcode;                // 0..0xF
    bool masked;
    std::vector<uint8_t> payload;  // UNMASKED application payload
};

inline std::vector<WsFrameVector> wsFrameVectors() {
    std::vector<WsFrameVector> v;
    for (auto& r : readRows("ws/frames.vectors")) {
        if (r.size() != 6)
            throw std::runtime_error("golden: ws frames expects 6 fields");
        WsFrameVector f;
        f.name = r[0];
        f.wire = hexDecode(r[1]);
        f.fin = (r[2] == "1");
        f.opcode = (uint8_t)hexNibble(r[3].empty() ? '0' : r[3][0]);
        f.masked = (r[4] == "1");
        f.payload = (r[5] == "<EMPTY>") ? std::vector<uint8_t>{}
                                        : hexDecode(r[5]);
