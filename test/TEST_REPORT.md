...
> Reproduce: `scripts/test-tui.py --isolate` (whole suite) — see
> [Methodology](#methodology) and [Reproducing](#reproducing).
...
- **The suite cannot complete at all under the legacy `run_tests.sh` shard
...
The legacy `run_tests.sh` splits the suite into 32 shards, each a single
...
3. **Masked flakiness.** The shard runner silently re-runs failed shards
   ...
   intermittent bug (`run_tests.sh:385`).
...
- **Stop masking flakiness** in `run_tests.sh` (the silent serial retry that
