...
TEST(ArrayLiteralPlacementTests, HeapByDefault) {
    auto jit = jitOf(
    ...
    int64_t r = fn(7);
    EXPECT_EQ(r / 1000, 0) << "bare literal was wrongly arena-allocated";
    EXPECT_GT(r % 1000, 0) << "bare heap literal did not touch the live-set";
}

// 3.1.2 — `stack [...]` parses and the values are correct.
TEST(ArrayLiteralPlacementTests, StackPrefixValues) {
    auto jit = jitOf(
        "    public static int32 run() {\n"
        "        int32[] xs = stack [10, 20, 30];\n"
...
TEST(ArrayLiteralPlacementTests, EscapingStackLiteralStaysHeap) {
    auto jit = jitOf(
        "    public static int32 first(int32 n) {\n"
        "        int32[] a = makeArr(n);\n"
        "        return a[0];\n"
        "    }\n"
        ...
        "        return a;\n"
        "    }\n");
    auto fn = jit->lookup<int32_t (*)(int32_t)>("first");
    EXPECT_EQ(fn(7), 107) << "escaping stack literal was wrongly arena-allocated";
}

// 3.1.4 — `shared [...]` is device-only; the host codegen path rejects it.
TEST(ArrayLiteralPlacementTests, SharedRejectedOnHost) {
    EXPECT_ANY_THROW(jitOf(
        "    public static int32 run() {\n"
        "        int32[] xs = shared [1, 2, 3];\n"
        "        return xs[0];\n"
        "    }\n"));
}
