...
// Vector tests do it — wrapping the matrix as a KernelBuffer<T> element so
// KernelBuffer<Matrix<...>> monomorphizes and lays out the matrix's LLVM type; the
...
TEST(MatrixTests, typeBooleanElementRejected) {
    std::string src = std::string("package test;\n") + IMPORTS +
        "public final class D {\n"
        "    public static int32 run() {\n"
        "        KernelBuffer<Matrix<boolean,2,2>> b = heap KernelBuffer<Matrix<boolean,2,2>>(0, 1);\n"
        "        return 0;\n"
        "    }\n"
        "}\n";
    try {
