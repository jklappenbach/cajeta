...
const char* kMatSource =
    ...
    "public class M {\n"
    "    @Kernel\n"
    "    public static void matk(GpuBuffer<float32> out, uint32 n) {\n"
...
const char* kDetInvSource =
    ...
    "public class M {\n"
    "    @Kernel\n"
    "    public static void detinv(GpuBuffer<float32> out, uint32 n) {\n"
...
const char* kMatMethodSource =
    ...
    "public class M {\n"
    "    @Kernel\n"
    "    public static void methk(GpuBuffer<float32> out, uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) {\n"
...
const char* kMatParamSource =
    ...
    "public class M {\n"
    "    @Kernel\n"
    "    public static void mpk(Matrix<float32,2,2> a, GpuBuffer<float32> out, uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) {\n"
...
TEST(XpuMatrixDeviceTests, matrixVectorRunsOnCpu) {
    const char* src =
        ...
        "public class M {\n"
        "    @Kernel\n"
        "    public static void mvk(GpuBuffer<float32> out, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            Matrix<float32,2,3> a = stack Matrix<float32,2,3>(1.0f,2.0f,3.0f,4.0f,5.0f,6.0f);\n"
