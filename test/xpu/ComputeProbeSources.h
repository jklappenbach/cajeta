...
inline const char* kNumericsBroadcastReduce() {
    ...
    "    @Kernel\n"
    "    public static void broadcastReduce(GpuBuffer<float32> acc,\n"
    "                                       GpuBuffer<float32> y, GpuBuffer<float32> x,\n"
    "                                       GpuBuffer<float32> scale,\n"
    "                                       GpuBuffer<float32> bias,\n"
    ...
    "        GpuBuffer<float32> sc = heap GpuBuffer<float32>(0, rows);\n"
    "        GpuBuffer<float32> bi = heap GpuBuffer<float32>(0, rows);\n"
    "        GpuBuffer<float32> y = heap GpuBuffer<float32>(0, n);\n"
    "        GpuBuffer<float32> acc = heap GpuBuffer<float32>(0, 1);\n"
...
inline const char* kTorchMatmulScatter() {
    ...
    "    @Kernel\n"
    "    public static void matmul(GpuBuffer<float32> C, GpuBuffer<float32> A,\n"
    "                              GpuBuffer<float32> B, uint32 M, uint32 K,\n"
    "                              uint32 N) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < M * N) {\n"
    "            uint32 row = i / N;\n"
    ...
    "    }\n"
    "    @Kernel\n"
    "    public static void scatterAdd(GpuBuffer<float32> grad, GpuBuffer<int32> idx,\n"
    "                                  GpuBuffer<float32> val, uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) {\n"
    ...
    "        GpuBuffer<float32> A = heap GpuBuffer<float32>(0, nA);\n"
    "        GpuBuffer<float32> B = heap GpuBuffer<float32>(0, nB);\n"
    "        GpuBuffer<float32> C = heap GpuBuffer<float32>(0, nC);\n"
    ...
    "        GpuBuffer<int32> idx = heap GpuBuffer<int32>(0, ns);\n"
    "        GpuBuffer<float32> val = heap GpuBuffer<float32>(0, ns);\n"
    "        GpuBuffer<float32> grad = heap GpuBuffer<float32>(0, P);\n"
...
inline const char* kSpelaFusedLayer() {
    ...
    "    @Kernel\n"
    "    public static void fusedLayer(GpuBuffer<float32> W, GpuBuffer<float32> x,\n"
    "                                  GpuBuffer<float32> t, GpuBuffer<float32> loss,\n"
    "                                  uint32 inDim, uint32 outDim, float32 lr) {\n"
    "        uint32 j = GpuThread.globalIdX();\n"
    "        if (j < outDim) {\n"
    "            float32 a = 0.0f;\n"
    ...
    "        GpuBuffer<float32> W = heap GpuBuffer<float32>(0, nW);\n"
    "        GpuBuffer<float32> x = heap GpuBuffer<float32>(0, inDim);\n"
    "        GpuBuffer<float32> t = heap GpuBuffer<float32>(0, outDim);\n"
    "        GpuBuffer<float32> loss = heap GpuBuffer<float32>(0, outDim);\n"
