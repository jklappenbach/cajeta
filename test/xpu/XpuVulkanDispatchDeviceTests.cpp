...
TEST(XpuVulkanDispatchDeviceTests, dynamicSharedOnDevice) {
    ...
    const char* src =
        ...
        "public class Dyn {\n"
        "    @Kernel\n"
        "    public static void dynstage(GpuBuffer<int32> out, GpuBuffer<int32> in,\n"
        "                                uint32 n) {\n"
        ...
        "        GpuBuffer<int32> in = heap GpuBuffer<int32>(0, n);\n"
        "        GpuBuffer<int32> out = heap GpuBuffer<int32>(0, n);\n"
        "        in.allocate();\n"
        "        out.allocate();\n"
        "        in.upload(hin);\n"
        "        out.upload(hout);\n"
...
TEST(XpuVulkanDispatchDeviceTests, gridStrideForEachOnDevice) {
    ...
    const char* src =
        ...
        "public class Grid {\n"
        "    @Kernel\n"
        "    public static void scale(GpuBuffer<float32> out, GpuBuffer<float32> in,\n"
        "                             uint32 n) {\n"
...
// Item 2 follow-up: a @Device helper taking GpuBuffer<T> params on the real Vulkan
...
TEST(XpuVulkanDispatchDeviceTests, deviceBufferParamHelperOnDevice) {
    ...
    const char* src =
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void k(GpuBuffer<float32> out, GpuBuffer<float32> in) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        scale(out, in, i);\n"
        "    }\n"
        "    public static float32 run() {\n"
...
TEST(XpuVulkanDispatchDeviceTests, podStructArgOnDevice) {
    ...
    const char* src =
        ...
        "public class PodArg {\n"
        "    @Kernel\n"
        "    public static void k(GpuBuffer<float32> out, Params p) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
...
TEST(XpuVulkanDispatchDeviceTests, textureSampleOnDevice) {
    ...
    const char* src =
        ...
        "        GpuBuffer<float32> us = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<float32> vs = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<float32> out = heap GpuBuffer<float32>(0, n);\n"
        "        us.allocate();\n"
        "        vs.allocate();\n"
        "        out.allocate();\n"
        "        us.upload(hus);\n"
...
static const char* kRgbaSampleSrc(const char* fmt) {
    ...
    s = std::string(
        ...
        "        GpuBuffer<float32> us = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<float32> vs = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<float32> out = heap GpuBuffer<float32>(0, m);\n"
...
static const char* kRgbaIntFetchSrc(const char* elem, const char* fmt) {
    ...
    s = std::string(
        ...
        "        GpuBuffer<" + e + "> out = heap GpuBuffer<" + e + ">(0, m);\n"
        "        out.allocate(); out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
        "        fetch.launch(s, grid: [1], block: [64])(tex, out, w, n);\n"
        "        s.sync();\n"
...
static const char* kTex3dSampleSrc() {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void samp(Texture3D vol, Sampler sn, GpuBuffer<float32> out,\n"
        ...
        "        GpuBuffer<float32> mo = heap GpuBuffer<float32>(0, 1);\n"
...
static const char* kTex2daSampleSrc() {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void samp(Texture2DArray arr, Sampler sn, GpuBuffer<float32> out,\n"
        "                            uint32 w, uint32 h, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 layer = i / (w*h);\n"
...
static const char* kTwoSamplersSrc() {
    ...
    s = std::string(
        ...
        "        GpuBuffer<float32> out = heap GpuBuffer<float32>(0, 2);\n"
        "        out.allocate(); out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
...
TEST(XpuVulkanDispatchDeviceTests, rayQuerySpatialIndexOnDevice) {
    ...
    const char* src =
        ...
        "                             GpuBuffer<float32> px, GpuBuffer<float32> py,\n"
        "                             GpuBuffer<float32> pz, GpuBuffer<uint32> out,\n"
        "                             uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            RayQuery rq;\n"
        ...
        "        hout[0] = 99; hout[1] = 99;\n"
        "        GpuBuffer<float32> px = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<float32> py = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<float32> pz = heap GpuBuffer<float32>(0, n);\n"
        "        GpuBuffer<uint32> out = heap GpuBuffer<uint32>(0, n);\n"
...
// GpuBuffer.slice (Stage B4) on Vulkan — device-verify the borrowing view-slot
...
// GpuBuffer.slice upload visibility through a kernel on Vulkan: a distinct pattern
...
// takes an ARRAY of buffers `GpuBuffer<int32>[] bufs` bound as one descriptor array
// and sums `bufs[b][i]` across `count` buffers indexed at runtime. GpuBuffer b is
...
// the Vulkan device. They lower to memory-only OpMemoryBarrier (proven spirv-val
