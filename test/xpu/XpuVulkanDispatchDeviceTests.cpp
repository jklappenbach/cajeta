...
TEST(XpuVulkanDispatchDeviceTests, dynamicSharedOnDevice) {
    ...
    const char* src =
        ...
        "public class Dyn {\n"
        "    @Kernel\n"
        "    public static void dynstage(KernelBuffer<int32> out, KernelBuffer<int32> in,\n"
        "                                uint32 n) {\n"
        ...
        "        KernelBuffer<int32> in = heap KernelBuffer<int32>(0, n);\n"
...
// Item 2 follow-up: a @Device helper taking KernelBuffer<T> params on the real Vulkan
...
static const char* kRgbaSampleSrc(const char* fmt) {
    ...
    s = std::string(
        ...
        "        KernelBuffer<float32> us = heap KernelBuffer<float32>(0, n);\n"
        "        KernelBuffer<float32> vs = heap KernelBuffer<float32>(0, n);\n"
        "        KernelBuffer<float32> out = heap KernelBuffer<float32>(0, m);\n"
...
static const char* kRgbaIntFetchSrc(const char* elem, const char* fmt) {
    ...
    s = std::string(
        ...
        "        KernelBuffer<" + e + "> out = heap KernelBuffer<" + e + ">(0, m);\n"
        "        out.allocate(); out.upload(hout);\n"
        "        KernelStream s = KernelStream.current();\n"
        "        fetch.launch(s, grid: [1], block: [64])(tex, out, w, n);\n"
        "        s.sync();\n"
...
static const char* kTex3dSampleSrc() {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void samp(Texture3D vol, Sampler sn, KernelBuffer<float32> out,\n"
        ...
        "        KernelBuffer<float32> mo = heap KernelBuffer<float32>(0, 1);\n"
...
static const char* kTex2daSampleSrc() {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void samp(Texture2DArray arr, Sampler sn, KernelBuffer<float32> out,\n"
        "                            uint32 w, uint32 h, uint32 n) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 layer = i / (w*h);\n"
...
static const char* kTwoSamplersSrc() {
    ...
    s = std::string(
        ...
        "        KernelBuffer<float32> out = heap KernelBuffer<float32>(0, 2);\n"
        "        out.allocate(); out.upload(hout);\n"
        "        KernelStream s = KernelStream.current();\n"
...
// and sums `bufs[b][i]` across `count` buffers indexed at runtime. KernelBuffer b is
