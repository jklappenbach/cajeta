...
TEST(XpuValueTypeOperatorDeviceTests, deviceEqualityOperatorDispatches) {
    std::string src =
        ...
        "public class M {\n"
        "    @Kernel\n"
        "    public static void eqk(Vec2 a, Vec2 b, KernelBuffer<float32> out, uint32 n) {\n"
...
TEST(XpuValueTypeOperatorDeviceTests, deviceInequalityDerivesFromEquality) {
    std::string src =
        ...
        "public class M {\n"
        "    @Kernel\n"
        "    public static void nek(Vec2 a, Vec2 b, KernelBuffer<float32> out, uint32 n) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            float32 r = 0.0f;\n"
        "            if (a != b) { r = 1.0f; }\n"
...
TEST(XpuValueTypeOperatorDeviceTests, deviceAggregateReturningOperatorDispatches) {
    std::string src =
        ...
        "public class M {\n"
        "    @Kernel\n"
        "    public static void addk(Vec2 a, Vec2 b, KernelBuffer<float32> out, uint32 n) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            Vec2 c = a + b;\n"
...
TEST(XpuValueTypeOperatorDeviceTests, deviceOperatorKernelValidatesAsSpirv) {
    std::string src =
        ...
        "public class M {\n"
        "    @Kernel\n"
        "    public static void addk(KernelBuffer<float32> out, uint32 n) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < n) {\n"
