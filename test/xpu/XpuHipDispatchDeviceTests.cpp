...
// Item 2 follow-up: a @Device helper taking GpuBuffer<T> params, on the real AMD
...
// GpuBuffer.slice (Stage B4) on HIP/AMD — device-verify the pointer-fold path. On
...
// GpuBuffer MemoryKind (Stage B4) on HIP/AMD — genuine zero-copy UNIFIED (managed)
...
// Bindless GpuBuffer<T>[] (descriptor-indexed buffers) on AMD/HIP. The compiler
...
// Stage 9: a kernel atomic with an explicit MemoryOrder.Relaxed runs on the AMD
