...
// real launch path:
//
//   GpuBuffer.allocate   -> __cajeta_xpu_buffer_alloc    -> malloc (CPU "device")
...
const char* kSaxpyLargeSource =
    ...
    "            hy[i] = 2.0f;\n"
    "        }\n"
    "        GpuBuffer<float32> x = heap GpuBuffer<float32>(n);\n"
...
const char* kGrid2dSource =
    ...
    "public class Grid2d {\n"
    "    @Kernel\n"
    "    public static void grid2d(GpuBuffer<int32> out) {\n"
...
const char* kBlock2dSource =
    ...
    "    @Kernel\n"
    "    public static void block2d(GpuBuffer<int32> out, uint32 w) {\n"
...
const char* kGridStrideForEachSource =
    ...
    "    @Kernel\n"
    "    public static void copy(GpuBuffer<float32> out, GpuBuffer<float32> in,\n"
    "                            uint32 n) {\n"
    ...
    "            hout[i] = -1.0f;\n"
    "        }\n"
    "        GpuBuffer<float32> in = heap GpuBuffer<float32>(n);\n"
...
const char* kDeviceHelperSource =
    ...
    "    @Kernel\n"
    "    public static void k(GpuBuffer<int32> out) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
...
// Item 2 follow-up — a @Device helper taking GpuBuffer<T> params. `scale` reads one
...
const char* kDeviceBufferParamSource =
    ...
    "public class DevBuf {\n"
    "    @Device\n"
    "    public static void scale(GpuBuffer<int32> out, GpuBuffer<int32> in,\n"
    "                             uint32 i) {\n"
    "        out[i] = in[i] * 3;\n"
    "    }\n"
    "    @Kernel\n"
    "    public static void k(GpuBuffer<int32> out, GpuBuffer<int32> in) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        scale(out, in, i);\n"
    "    }\n"
    "    public static int32 run() {\n"
...
// GpuBuffer.slice (Stage B4) — a non-owning sub-buffer view passed to a kernel.
...
const char* kBufferSliceSource =
    ...
    "public class Slice {\n"
    "    @Kernel\n"
    "    public static void fill(GpuBuffer<int32> b, uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) { b[i] = (int32) i; }\n"
    "    }\n"
    "    public static int32 run() {\n"
    ...
    "        GpuBuffer<int32> all = heap GpuBuffer<int32>(n);\n"
    "        all.upload(h);\n"
    "        GpuBuffer<int32> tail = all.slice(half, half);\n"
    "        GpuStream s = GpuStream.current();\n"
...
// GpuBuffer.slice upload visibility through a kernel: a distinct pattern is
...
const char* kBufferSliceUploadSource =
    ...
    "    @Kernel\n"
    "    public static void dbl(GpuBuffer<int32> b, uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) { b[i] = b[i] * 2; }\n"
    "    }\n"
    "    public static int32 run() {\n"
    ...
    "        GpuBuffer<int32> sub = all.slice(off, len);\n"
    "        sub.upload(mid);\n"
    "        GpuStream s = GpuStream.current();\n"
...
static const char* kRgbaSampleSrcCpu(const char* fmt) {
    ...
    s = std::string(
        ...
        "        GpuBuffer<float32> us = heap GpuBuffer<float32>(n);\n"
        "        GpuBuffer<float32> vs = heap GpuBuffer<float32>(n);\n"
        "        GpuBuffer<float32> out = heap GpuBuffer<float32>(m);\n"
...
// `tex.fetch(x, y)` — unfiltered, no Sampler. GpuThread i decodes its (x, y) from
...
static const char* kRgbaFetchSrcCpu(const char* fmt) {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture2D tex, GpuBuffer<float32> out,\n"
        "                             uint32 w, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 y = i / w;\n"
...
static const char* kRgbaIntFetchSrcCpu(const char* elem, const char* fmt) {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture2D<") + e + "> tex, GpuBuffer<" + e + "> out,\n"
        "                             uint32 w, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 y = i / w;\n"
        ...
        "        GpuBuffer<" + e + "> out = heap GpuBuffer<" + e + ">(m);\n"
        "        out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
        "        fetch.launch(s, grid: [1], block: [n])(tex, out, w, n);\n"
        "        s.sync();\n"
...
static const char* kR32iFetchSrcCpu() {
    ...
    s =
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture2D<int32> tex, GpuBuffer<int32> out,\n"
        "                             uint32 w, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 y = i / w;\n"
        ...
        "        GpuBuffer<int32> out = heap GpuBuffer<int32>(n*2);\n"
        "        out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
        "        fetch.launch(s, grid: [1], block: [n])(tex, out, w, n);\n"
        "        s.sync();\n"
...
static const char* kTex3dFetchSrcCpu() {
    ...
    s =
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture3D vol, GpuBuffer<float32> out,\n"
...
static const char* kTex3dSampleSrcCpu() {
    ...
    s =
        ...
        "                            GpuBuffer<float32> out, uint32 w, uint32 h,\n"
        "                            uint32 d, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 z = i / (w*h);\n"
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void mid(Texture3D vol, Sampler sl, GpuBuffer<float32> out) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < 1) {\n"
        ...
        "        GpuBuffer<float32> mout = heap GpuBuffer<float32>(1);\n"
        "        mout.upload(hmid);\n"
...
static const char* kTex3dIntFetchSrcCpu(const char* elem, const char* fmt) {
    ...
    s = std::string(
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture3D<") + e + "> vol, GpuBuffer<" + e + "> out,\n"
        "                             uint32 w, uint32 h, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 z = i / (w*h);\n"
...
static const char* kTex1dFetchSrcCpu() {
    ...
    s =
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture1D row, GpuBuffer<float32> out, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
...
static const char* kTex1dSampleSrcCpu() {
    ...
    s =
        ...
        "    @Kernel\n"
        "    public static void samp(Texture1D row, Sampler sn, GpuBuffer<float32> out,\n"
        "                            uint32 w, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void mid(Texture1D row, Sampler sl, GpuBuffer<float32> out) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < 1) {\n"
...
static const char* kTex2daFetchSrcCpu() {
    ...
    s =
        ...
        "    @Kernel\n"
        "    public static void fetch(Texture2DArray arr, GpuBuffer<float32> out,\n"
        "                             uint32 w, uint32 h, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 layer = i / (w*h);\n"
...
static const char* kTex2daSampleSrcCpu() {
    ...
    s =
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void mid(Texture2DArray arr, Sampler sl, GpuBuffer<float32> out) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < 1) {\n"
...
static const char* kTwoSamplersSrcCpu() {
    ...
    s =
        ...
        "        GpuBuffer<float32> out = heap GpuBuffer<float32>(2);\n"
        "        out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
...
static const char* kMipSrcCpu() {
    ...
    s =
        ...
        "public class MipCpu {\n"
        "    @Kernel\n"
        "    public static void mip(Texture2D tex, Sampler sl, GpuBuffer<float32> out) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < 1) {\n"
        ...
        "        GpuBuffer<float32> out = heap GpuBuffer<float32>(3);\n"
        "        out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
...
// Item 2 follow-up: a @Device helper taking GpuBuffer<T> params runs on CPU — buffer
...
// Stage 11: labeled break/continue in kernels (break/continue to an OUTER loop).
// The results are chosen so a labeled jump differs from an innermost one:
//   `break outer` at (i==g, j==3) exits BOTH loops → acc = g*8 + 4
//     (innermost break would keep iterating outer rows and over-count).
//   `continue outer` at j==2 skips the rest of the inner loop AND the outer
//     body's trailing `+100` → acc = 2 per row × 4 = 8
...
const char* kLabeledLoopSource =
    ...
    "    @Kernel\n"
    "    public static void brk(GpuBuffer<int32> out, uint32 n) {\n"
    ...
    "    }\n"
    "    @Kernel\n"
    "    public static void cont(GpuBuffer<int32> out, uint32 n) {\n"
    "        uint32 g = GpuThread.globalIdX();\n"
    "        if (g < n) {\n"
    "            int32 acc = 0;\n"
    ...
    "        GpuBuffer<int32> ob = heap GpuBuffer<int32>(n);\n"
    "        GpuBuffer<int32> oc = heap GpuBuffer<int32>(n);\n"
    "        GpuStream s = GpuStream.current();\n"
...
TEST(XpuCpuDispatchTests, labeledBreakContinueOnCpu) {
    auto jit = CajetaJit::compile(kLabeledLoopSource, "test.LabLoop", cpuOptions());
    ASSERT_NE(jit, nullptr);
    auto fn = jit->lookup<int (*)()>("run");
    ASSERT_NE(fn, nullptr);
    int r = fn();
...
// GpuBuffer.slice — a non-owning sub-view passed to a kernel writes through the
...
// GpuBuffer.slice — upload/download through a mid-buffer view honor the byte
...
// GpuBuffer MemoryKind (Stage B4) — a host-accessible (Unified) buffer with
...
const char* kMemKindUnifiedSource =
    ...
    "    @Kernel\n"
    "    public static void inc(GpuBuffer<int32> b, uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) { b[i] = b[i] + 1; }\n"
    "    }\n"
    "    public static int32 run() {\n"
    ...
    "        GpuBuffer<int32> u = heap GpuBuffer<int32>(0, n);\n"
...
// CPU (portability). GpuStream.create() is the default stream (handle 0) on the CPU
...
const char* kAsyncPipelineSource =
    ...
    "        int32[] h = heap int32[n];\n"
    "        for (uint32 i = 0; i < n; i = i + 1) { h[i] = (int32) i; }\n"
    "        GpuBuffer<int32> b = heap GpuBuffer<int32>(n);\n"
    "        GpuStream s = GpuStream.create();\n"
    "        b.uploadAsync(h, s);\n"
...
const char* kEventFenceSource =
    ...
    "        for (uint32 i = 0; i < n; i = i + 1) { h[i] = (int32) i; }\n"
    "        GpuBuffer<int32> b = heap GpuBuffer<int32>(n);\n"
    "        GpuStream s1 = GpuStream.create();\n"
    "        GpuStream s2 = GpuStream.create();\n"
...
// The same GpuBuffer<int32>[] gather kernel: bufs[b][i] across `count` buffers
...
// GpuBuffer b = (b+1)*10 + i, so out[i] = 60 + 3i for 3 buffers. (The real bindless
...
const char* kBindlessSource =
    ...
    "    @Kernel\n"
    "    public static void gather(GpuBuffer<int32>[] bufs, uint32 count,\n"
    ...
    "        uint32 n = 64;\n"
    "        uint32 k = 3;\n"
    "        GpuBuffer<int32>[] bufs = heap GpuBuffer<int32>[k];\n"
    "        for (uint32 b = 0; b < k; b = b + 1) {\n"
    "            int32[] h = heap int32[n];\n"
    "            for (uint32 i = 0; i < n; i = i + 1) {\n"
    "                h[i] = (int32)((b + 1) * 10 + i);\n"
    "            }\n"
    "            bufs[b] = heap GpuBuffer<int32>(n);\n"
    "            bufs[b].upload(h);\n"
    "        }\n"
    "        GpuBuffer<int32> out = heap GpuBuffer<int32>(n);\n"
    "        GpuStream s = GpuStream.current();\n"
...
TEST(XpuCpuDispatchTests, memoryFenceOnCpu) {
    const char* src =
        ...
        "    public static int32 run() {\n"
        "        uint32 n = 64;\n"
        "        GpuBuffer<int32> data = heap GpuBuffer<int32>(n);\n"
        "        GpuBuffer<int32> out = heap GpuBuffer<int32>(n);\n"
        "        GpuStream s = GpuStream.current();\n"
...
TEST(XpuCpuDispatchTests, relaxedAtomicCounterOnCpu) {
    const char* src =
        ...
        "public class RAC {\n"
        "    @Kernel\n"
        "    public static void count(GpuBuffer<int32> out, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
        "        if (i < n) {\n"
...
TEST(XpuCpuDispatchTests, specConstantFloatBakesDefaultOnCpu) {
    const char* src =
        ...
        "public class SCF {\n"
        "    @Kernel\n"
        "    public static void fill(GpuBuffer<float32> out, uint32 n) {\n"
        "        uint32 i = GpuThread.globalIdX();\n"
