...
// real launch path:
//
//   KernelBuffer.allocate   -> __cajeta_xpu_buffer_alloc    -> malloc (CPU "device")
...
// Item 2 follow-up — a @Device helper taking KernelBuffer<T> params. `scale` reads one
...
// KernelBuffer.slice (Stage B4) — a non-owning sub-buffer view passed to a kernel.
...
// KernelBuffer.slice upload visibility through a kernel: a distinct pattern is
...
static const char* kRgbaSampleSrcCpu(const char* fmt) {
    ...
    s = std::string(
        ...
        "        KernelBuffer<float32> us = heap KernelBuffer<float32>(n);\n"
        "        KernelBuffer<float32> vs = heap KernelBuffer<float32>(n);\n"
        "        KernelBuffer<float32> out = heap KernelBuffer<float32>(m);\n"
...
// `tex.fetch(x, y)` — unfiltered, no Sampler. KernelThread i decodes its (x, y) from
...
static const char* kRgbaIntFetchSrcCpu(const char* elem, const char* fmt) {
    ...
    s = std::string(
        ...
        "        KernelBuffer<" + e + "> out = heap KernelBuffer<" + e + ">(m);\n"
        "        out.upload(hout);\n"
        "        KernelStream s = KernelStream.current();\n"
        "        fetch.launch(s, grid: [1], block: [n])(tex, out, w, n);\n"
        "        s.sync();\n"
...
static const char* kR32iFetchSrcCpu() {
    ...
    s =
        ...
        "        KernelBuffer<int32> out = heap KernelBuffer<int32>(n*2);\n"
        "        out.upload(hout);\n"
        "        KernelStream s = KernelStream.current();\n"
        "        fetch.launch(s, grid: [1], block: [n])(tex, out, w, n);\n"
        "        s.sync();\n"
...
static const char* kTex3dSampleSrcCpu() {
    ...
    s =
        ...
        "                            KernelBuffer<float32> out, uint32 w, uint32 h,\n"
        "                            uint32 d, uint32 n) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < n) {\n"
        "            uint32 z = i / (w*h);\n"
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void mid(Texture3D vol, Sampler sl, KernelBuffer<float32> out) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < 1) {\n"
        ...
        "        KernelBuffer<float32> mout = heap KernelBuffer<float32>(1);\n"
        "        mout.upload(hmid);\n"
...
static const char* kTex1dSampleSrcCpu() {
    ...
    s =
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void mid(Texture1D row, Sampler sl, KernelBuffer<float32> out) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < 1) {\n"
...
static const char* kTex2daSampleSrcCpu() {
    ...
    s =
        ...
        "    }\n"
        "    @Kernel\n"
        "    public static void mid(Texture2DArray arr, Sampler sl, KernelBuffer<float32> out) {\n"
        "        uint32 i = KernelThread.globalIdX();\n"
        "        if (i < 1) {\n"
...
const char* kLabeledLoopSource =
    ...
    "    }\n"
    "    @Kernel\n"
    "    public static void cont(KernelBuffer<int32> out, uint32 n) {\n"
    "        uint32 g = KernelThread.globalIdX();\n"
    "        if (g < n) {\n"
    "            int32 acc = 0;\n"
    ...
    "        KernelBuffer<int32> oc = heap KernelBuffer<int32>(n);\n"
    "        KernelStream s = KernelStream.current();\n"
...
// KernelBuffer b = (b+1)*10 + i, so out[i] = 60 + 3i for 3 buffers. (The real bindless
