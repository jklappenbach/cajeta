...
// A @Device helper taking GpuBuffer<T> params, on the real NVIDIA device. The kernel
...
// GpuBuffer.slice on CUDA — device-verify the pointer-fold path. The device handle
...
// Bindless GpuBuffer<int32>[] (descriptor-indexed buffers) on CUDA. The launch
...
TEST(XpuCudaDispatchDeviceTests, specOverridePartialReadsDefaultForUnsetSlotsOnCuda) {
    ...
    const char* src =
        ...
        "        GpuBuffer<int32> out = heap GpuBuffer<int32>(0, n*2);\n"
        "        out.allocate();\n"
        "        out.upload(hout);\n"
        "        GpuStream s = GpuStream.current();\n"
