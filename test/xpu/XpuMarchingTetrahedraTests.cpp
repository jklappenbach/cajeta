//
// XpuMarchingTetrahedraTests — cajeta-gfx plan §2.b, the isosurface extractor
// (cajeta.xpu.MarchingTetrahedra). Marching tetrahedra is used in place of
...
const char* IMP = "import cajeta.xpu.MarchingTetrahedra;\n";
...
TEST(XpuMarchingTetrahedraTests, emptyCellsEmitNoTriangles) {
...
TEST(XpuMarchingTetrahedraTests, singleCornerInsideEmitsSixPatches) {
    EXPECT_EQ(runI32(IMP,
        "        float32[] field = heap float32[8];\n"
        "        int32 i = 0;\n"
        "        while (i < 8) { field[i] = 1.0f; i = i + 1; }\n"
...
TEST(XpuMarchingTetrahedraTests, spherePolygonizesOntoSurface) {
    EXPECT_EQ(runI32(IMP,
        "        int32 N = 9;\n"
        "        float32 h = 0.25f;\n"
        "        float32 rad = 0.6f;\n"
