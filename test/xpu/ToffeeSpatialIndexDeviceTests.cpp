...
const char* kDriver =
    ...
    "        GpuBuffer<float32> qx = heap GpuBuffer<float32>(0, n);\n"
    "        GpuBuffer<float32> qy = heap GpuBuffer<float32>(0, n);\n"
    "        GpuBuffer<float32> qz = heap GpuBuffer<float32>(0, n);\n"
...
const char* kExactDriver =
    ...
    "        GpuBuffer<float32> dx = heap GpuBuffer<float32>(0, np);\n"
    "        GpuBuffer<float32> dy = heap GpuBuffer<float32>(0, np);\n"
    "        GpuBuffer<float32> dz = heap GpuBuffer<float32>(0, np);\n"
    ...
    "        GpuBuffer<uint32> oExact = heap GpuBuffer<uint32>(0, n);\n"
    "        GpuBuffer<uint32> oBox = heap GpuBuffer<uint32>(0, n);\n"
...
const char* kRqMinDriver =
    ...
    "        GpuBuffer<float32> qx = heap GpuBuffer<float32>(n);\n"
    "        GpuBuffer<float32> qy = heap GpuBuffer<float32>(n);\n"
    "        GpuBuffer<float32> qz = heap GpuBuffer<float32>(n);\n"
...
const char* kTriDriver =
    ...
    "                                GpuBuffer<float32> ox, GpuBuffer<float32> oy,\n"
    ...
    "        GpuBuffer<float32> ox = heap GpuBuffer<float32>(n);\n"
    "        GpuBuffer<float32> oy = heap GpuBuffer<float32>(n);\n"
    "        GpuBuffer<uint32> out = heap GpuBuffer<uint32>(n);\n"
...
const char* kNearestDriver =
    ...
    "        GpuBuffer<float32> outT = heap GpuBuffer<float32>(1);\n"
    "        GpuBuffer<uint32> outI = heap GpuBuffer<uint32>(2);\n"
...
const char* kFrontDriver =
    ...
    "        GpuBuffer<float32> oz = heap GpuBuffer<float32>(n);\n"
    "        GpuBuffer<float32> dz = heap GpuBuffer<float32>(n);\n"
    "        GpuBuffer<uint32> out = heap GpuBuffer<uint32>(n);\n"
...
const char* kOptixImplDriver =
    ...
    "    @Kernel\n"
    "    public static void noop(GpuBuffer<uint32> b) {\n"
    "        uint32 i = GpuThread.globalIdX(); if (i == 0) { b[0] = 1; }\n"
    "    }\n"
    "    public static int32 run() {\n"
    "        uint32 np = 3;\n"
    "        float32[] boxes = heap float32[np * 6];\n"
...
const char* kSelectDriver =
    ...
    "        }\n"
    "    }\n"
    "    @Kernel\n"   // UNSUPPORTED shape (AS, 2 GpuBuffer, 1 scalar) -> software floor
    "    public static void countOne(AccelerationStructure scene,\n"
    "                                GpuBuffer<float32> qx, GpuBuffer<uint32> out,\n"
    "                                uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
    "        if (i < n) {\n"
    "            RayQuery rq;\n"
...
const char* kUnsupOptixDriver =
    ...
    "    @Kernel\n"   // (AS, 2 GpuBuffer, 1 scalar) -> Unsupported -> software floor
    "    public static void countOne(AccelerationStructure scene,\n"
    "                                GpuBuffer<float32> qx, GpuBuffer<uint32> out,\n"
    "                                uint32 n) {\n"
    "        uint32 i = GpuThread.globalIdX();\n"
...
// AABB AS and launch an UNSUPPORTED-shape kernel (AS, 2 GpuBuffer, 1 scalar → software cubin
