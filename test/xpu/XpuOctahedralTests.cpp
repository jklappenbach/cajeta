//
// XpuOctahedralTests — cajeta-gfx plan §3 (slice 3c): octahedral unit-vector
// encoding (cajeta.xpu.Octahedral).
//
...
TEST(XpuOctahedralTests, cardinalAxesRoundTrip) {
...
TEST(XpuOctahedralTests, upperHemisphereRoundTrip) {
    EXPECT_EQ(runI32(IMP,
        "        float32 x = 1.0f;\n"
        "        float32 y = 2.0f;\n"
        "        float32 z = 3.0f;\n"
...
TEST(XpuOctahedralTests, lowerHemisphereRoundTrip) {
    EXPECT_EQ(runI32(IMP,
        "        float32 x = 1.0f;\n"
        "        float32 y = 0.0f - 2.0f;\n"
        "        float32 z = 0.0f - 2.0f;\n"
...
TEST(XpuOctahedralTests, encodedInRangeDecodedUnit) {
    EXPECT_EQ(runI32(IMP,
        "        int32 i = 0;\n"
        "        while (i < 512) {\n"
        "            int32 a = i % 8;\n"
